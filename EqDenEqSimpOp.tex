...
{\color{blue}
  \todo[inline]{Fixed. Combine with ``CHAR PLAYGROUND SECTION''}
\begin{definition}
      ...
      Fix an inner edge $e \in U_0$.
      We say that $K e \subseteq U_0$ is a \textit{characteristic edge orbit} of
      $U_0$ with respect to $X$
      if
      \begin{enumerate}
      \item $X$ contains all proper outer faces of $U_0$; and
      \item for any $R_0 \in \mathscr{F}_{\mathrm{Inn}}(U_0)$ containing $K e$ as inner edges,
            $R_0 \in \cap X$
            if and only if
            $R_0 - K e \in \cap X$.
...
\begin{proposition}
      ...
      If $K e$ is a characteristic edge orbit of $U_0$ with respect to $X$, then
...
\begin{proof}
      ...
      \begin{equation}
            X_C
            := X \cup \mathop{\bigcup}\limits_{C}\Omega[U_0],
      \end{equation}
      % \begin{equation}
      %       X_C
      %       := X \cup \mathop{\bigcup}\limits_{C}\Omega[U_0],
      %       %       := X \cup \mathop{\bigcup}\limits_{C/K} \mathsf{im}\left(\Omega[G \cdot_L U_0 \setminus E] \right)
      %       %       = X \cup \left( G \cdot_K \mathop{\bigcup}\limits_{C/K} \Omega[U_0 \setminus E] \right),
      % \end{equation}
      with the unions taking place in $\Omega[T]$
      and $\mathscr{F}_{\mathrm{Inn}}^{K e}(U_0)$ the poset of inner faces $V_0$ of $U_0$ with $K e \subseteq V_0$.
      Moreover, it suffices to consider when $C' = C \cup G\cdot_K V_0$;
      ...
      \begin{equation}
            \label{CHAR_HORN_EQ}
            G \cdot_{\bar K} \left( \Lambda^{K e}[V_0] \to \Omega[V_0] \right),
      \end{equation}
      which by \cite[Proposition 6.17]{Per17} is inner $G$-anodyne.     
      \todo[inline]{don't need to worry about the maps of the pushout being injective!}
      
      We first show that $\Lambda^{K e}[V_0]$ is in $X_C$.
      If $S_0$ is an outer face of $ V_0$, then
      $S_0$ factors through an outer face of $U_0$, and so $S_0$ is in $X$.
      Further, if $S_0 = V_0 \setminus E$ with $E \cap K e = \varnothing$,
      then concavity implies that $S_0$ is in the domain, as required.

      Lastly, we show that no face $S_0 =  V_0 \setminus \bar e = U_0 \setminus (D \cup \bar e)$,
      with $\bar e \subseteq K e$,
      is in $X_C$.
      If $S_0$ is contained in some $U_0 \setminus E$ already attached,
      then, since $E \cap K e = \varnothing$ we have $U_0 \setminus D \subseteq U_0 \setminus E$, so
      $U_0 \setminus D = V_0$ is also in the domain, a contradiction.
      On the other hand,
      if $V_0 \setminus \bar e$ is in $X$,
      then so is $V_0 \setminus K e$, and hence
      so is $V_0$ (by definition of characteristic edge orbit), also a contradiction.
\end{proof}

The following observation will be useful in the following proofs.
\begin{lemma}
      \label{FOF_IN_OHORN_LEM}
      Given $T \in \Omega_G$, and a face $V_0 \into T$. Then
      $V_0 \in \Lambda^{G e}_o[T]$ if and only if $V_0 - G e \in \Lambda^{G e}_o[T]$. 
\end{lemma}
\begin{proof}
      This follows from \cref{GINNER REM} and the existence of a minimal orbital face $G V_0$ containing $V_0$.
      Indeed, we have a commuting diagram
      \begin{equation}
            \begin{tikzcd}
                  V_0 - G e \arrow[r] \arrow[d]
                  &
                  G V_0 - G e \arrow[d]
                  \\
                  V_0\arrow[r]
            ...
            \end{tikzcd}
      \end{equation}
      % where $K_r$ is the stabilizer of $r(V_0)$,
      and $G V_0$ (and hence $G V_0 - G e$) is the minimal orbital face containing $V_0$ (resp. $V_0 - K e$)\footnote{
        Here we are using that $G V_0 - G e = G V_0 - K_r e$, where $K_r$ is the stabilizer of $r(V_0)$}.
      It is immediate that both for any face $S_0$ of $T$ that
      \begin{itemize}
      \item $S_0 \in \Lambda^{G e}_o[T]$ iff $G S_0$ is; and
      \item $G S_0 \in \Lambda^{G e}_o[T]$ iff $G S_0 - K_r e$ is.
      \end{itemize}
      Thus we may conclude that
      $V_0 - G e$ is in the orbital horn $\Lambda^{G e}_o[T]$
      iff
      $V_0$ is as well.
\end{proof}
...
\begin{proof}
      ...
      To that end, let $V_0 := U_0 \setminus D$ be an inner face
      with $D \cap K e = \varnothing$ and stabilizer $\bar K$.
      
      The case $B = \varnothing$ follows immediately from \cref{FOF_IN_OHORN_LEM}.
      % , we have a commuting diagram
      % \begin{equation}
      %       \begin{tikzcd}
      %             V_0 - K e \arrow[r] \arrow[d]
      %             &
      %             G V_0 - K_r e \arrow[d]
      %             \\
      %             V_0\arrow[r]
      ...
      %       \end{tikzcd}
      % \end{equation}
      % where $K_r = \mathrm{Stab}_G(r(V_0))$,
      % and $G V_0$ (and hence $G V_0 - K_r e$) is the minimal orbital face containing $V_0$ (resp. $V_0 - K e$);
      % see \cref{GINNER REM}.
      % As $S_0 \in \Lambda^{G e}_o[T]$ iff $G S_0$ is,
      % and $G S_0 \in \Lambda^{G e}_o[T]$ iff $G S_0 - K_r e$ is,
      % we may conclude that
      % $V_0 - K e$ is in the orbital horn $\Lambda^{G e}_o[T]$
      % iff
      % $V_0$ is as well.
      
      %%%%%% -----------------------  WRONG ------------------------------------------------
      % $U_0 \setminus (D \cup \bar K e) \in \Lambda_o^{G e}[T]$. This implies that
      % \begin{equation}
      ...
      For general $B$, we know the domain of \eqref{ORB_HORN_EQ}
      contains all outer faces of $U_0$ by convexity.
      Now, let $V_0 := U_0 - D$ be an inner face, with stabilizer $\bar K$.
      Then $V_0 - K e$ is in the domain implies it is contained in either 
      $\Lambda_o^{Ge}[T]$ or
      some outer face $R_0$ already attached.
      In the former case, we again apply \cref{FOF_IN_OHORN_LEM} to conclude that $V_0$ is in the orbital horn;
      in the latter case, we may conclude that $U_0$ is contained in $R_0$ since both are outer faces, yielding a contradiction.
      ...
      % Now, let $V_0 := U_0 \setminus D$ be an inner face, with stabilizer $\bar K$.
...
\begin{proof}
      ...
      We go by induction on $|H| \times |V_G(T)|$, ordered lexicographically. \todo{replace $F_{Inn}(T)$ or $Inn(T)$ with $Inn_G(T)$? $E(T)$? $V(T)$? $V_G(T)$?}
      ...
      (since if $K = H$, $U_0$ being a proper face implies it has fewer vertices). COME BACK 
...
Given a set $\Xi$ of inner edges of $T$ and a subface $V \hookrightarrow U$, denote $\Xi_V = \Xi \cap Inn(V)$.
...
\begin{remark}
	...
	condition (Ch3) can be deduced from a much simpler condition. Namely, is suffices to check that:
	\begin{itemize}
...
To check (a), writing $\bar{W}$ for the corresponding outer face of $U$, one has
	\[
	\Xi_W = \Xi \cap Inn (W) 
	= \Xi \cap Inn(V') \cap Inn(\bar{W})
	= \Xi \cap Inn(\bar{V}') \cap Inn(\bar{W})
	= \Xi \cap Inn(\bar{W})
	= \Xi_{\bar{W}}
	\]
...
the new (Ch2) condition holds for the \textit{any possible choice} of $J$).
\end{proof}
...
\begin{itemize}
\item[(i)] In \cite[\S 10]{Rez01}, Rezk introduces the notion of \textit{covers}, which in our language are the subsets
...
is $G$-inner anodyne. 
Indeed, letting $\Xi = Inn(T)$ and $\{U_i\}$ be the set of closed faces $U = \bar{U}^T$ in $X'$, (Ch0) is obvious, (Ch1) follows since 
...
$E_{\**}=E \cap Inn(T_{\**})$, $F_{\**} = F \cap Inn(T_{\**})$,
...
\begin{proposition}[{c.f. \cite[Proposition 6.17]{Per17}}]
      \label{GEN_GHORN_THM}
      Generalized inner horn inclusions
      \begin{equation}
            \Lambda^A[T] \to \Omega[T]
      \end{equation}
are inner $G$-anodyne.
\end{proposition}
\begin{proof}
      Picking a decomposition $T \simeq G \cdot_H T_0$, we apply Lemma \ref{CHAREDGE LEM} with
      $U = T_0$, $E = A \cap T_0$, and $X = \Lambda^{A}[T]$;
      all three verifications that $E$ is a characteristic edge set with respect to $X$ are immediate.
...
\begin{proof}
The case $E=\emptyset$ is tautological.
