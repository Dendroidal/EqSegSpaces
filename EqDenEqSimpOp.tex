...
\begin{inparaenum}
	...
	\item[(iii)] and lastly, \cite{CM13b} established the existence of the model structure on $\mathsf{sOp}$ as well as the Quillen equivalence between $\mathsf{sOp}$ and $\mathsf{PreOp}$, finishing the proof of the main result.
\end{inparaenum}
...
are ``equivariant operads with norms up to homotopy'',
...
But now since $W' \hookrightarrow V$ it is $ \bar{W} = \bar{W}' \hookrightarrow \bar{V}$, so that by convexity of $C'$ it must be 
...
so that not only $W - \Xi_{W} = W' \hookrightarrow V$ but also $W \hookrightarrow V$.
But hence $W \leq V$ and convexity of $C$ implies 
$W \in C$, a contradiction. 
