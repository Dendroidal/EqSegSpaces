...
\begin{proposition}\label{COMBMODSTR PROP}
	...
	$(C,W,F)$ and for any $(C,W,F)$-fibrant 
	objects $c,d \in \mathcal{C}$ one has that $c\to d$ is in $W$ iff it is in $W_1$ iff it is in $W_2$.
	
	Moreover, an object $c \in \mathcal{C}$ is $(C,W,F)$-fibrant iff it is simultaneously 
...
\begin{proof}
	The joint model structure $(C,W,F)$ can be obtained by either localizing left Bousfield localizing $(C,W_1,F_1)$ with regards to the generating trivial cofibrations of $(C,W_2,F_2)$ or vice-versa. That the two processes yield the same model structure follows from from the universal property of left Bousfield localizations \cite[Prop. 3.4.18]{Hir03}.
	
	The claim concerning joint fibrant $c,d$ follows from the local Whitehead theorem \cite[Thm. 3.3.8]{Hir03}, stating that
...
\subsection{Equivariant pre-operads}\label{PREOP SEC}
