...
\begin{itemize}
	...
	\item[(iv)] the canonical map $X_{\bullet,0} \to \delta^{\**}(X)$ is a Kan equivalence. % $X_{n,0} \to X_{n,n}$
\end{itemize}
\end{proposition}
...
is left Quillen for either the horizontal or vertical Reedy structures (and thus also for the joint Reedy structure). But noting that all objects are cofibrant, and regarding % $X_{n,0}$
...
{\color{purple}
We extend Notations \ref{HOM_NOTATIONS} to the bisimplicial and simplicial-dendroidal world.
\begin{notation}
      Here, we will abuse notation, and let
      $\mathsf{dSet}$ denote either of $\mathsf{sSet}$ or $\mathsf{dSet}$;
      in particular, it will denote the horizontal component of the bi-graded objects.
      % Similarly, let $\pi_s$ and $\pi_d$ denote the obvious projections.
      Moreover, let $\hat\times$ denote the \textit{external product}.
      % Finally, let $y(-)$ denote the natural Yoneda embeddings.
      
      Now fix $A \in \mathsf{dSet}^G$,      
      $K \in \mathsf{sSet}$, and
      $Y \in \mathsf{sdSet}^G$. We define
      \begin{itemize}
      \item $Y^K \in \mathsf{dSet}$ by $Y^K(T) = \mathsf{sSet}(\Omega[T] \hat\times K, Y)$.
      \item $Y^{A} \in \mathsf{sSet}$ by $Y^A(n) = \mathsf{dSet}(A \hat\times \Delta[n],Y)$.
      \item $Y^{(A)} \in \mathsf{sdSet}_G$ by $Y^{(A)}(T) = k(i^{\**}\Hom(\Omega[T] \otimes Y,X) \in \mathsf{sSet}$.
      \end{itemize}
\end{notation}
}
