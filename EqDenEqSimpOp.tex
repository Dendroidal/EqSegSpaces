...
\begin{notation}
      Given a class of maps $\mathcal C$ in $\mathcal V$, let $^\boxslash\mathcal C$ (respecitvely $^{\boxslash !}\mathcal C$)
...
\begin{lemma}
      ...
      ${}^\boxslash\mathcal C = {}^\boxslash W(\mathcal C)$
      and
      ${}^{\boxslash !}\mathcal C = {}^{\boxslash !} \hat{W}(\mathcal C)$.
\end{lemma}
\begin{proof}
...
\begin{proof}
      ...
      \end{equation}
      Thus $N^G(\O)$ is in $SRLP(SCI_G)$, and hence is in $SRLP(IHI_G)$
      by Lemma \ref{HYPER_LP_LEM} and Proposition \ref{GHORN_IN_SC_PROP}.

      Conversely, by Lemma \ref{SRLP_IHI_LEM} and \cite[Theorem 6.1, Proposition 6.10]{MW07}, we have that
...
For the only if direction, form the completion 
$\gamma^{\**}X \to \tilde{X}$
as in Proposition \ref{COMPLE PROP}.
The adjoint map $X \to \gamma_{\**}\tilde{X}$,
where $\gamma_{\**}\tilde{X} \in \mathsf{PreOp}^G$
