...
\begin{remark}
Our definition of $G$-$\infty$-operads is a priori distinct from the original definition \cite[Def. 6.12]{Per17} which used only 
\textit{generating $G$-inner horns}.
...
\begin{notation}
      The saturation of the maps in \eqref{JOINTCOF EQ} is the class of \textit{normal monomorphisms} in $\mathsf{sdSet}^G$.
\end{notation}
...
\end{lemma}
\todo[inline]{what is a normal monomorphism in $\mathsf{sdSet}^G$?}

\begin{proof}
	Using the cellular filtration in $\mathsf{sdSet}^G$, 
	any {\color{blue} normal} monomorphism $A \to B$ in $\mathsf{PreOp}^G$ can be written as a transfinite composition of pushouts of maps in 
...
\todo[inline]{fill in about $J_d$-equivs}
\begin{equation}
