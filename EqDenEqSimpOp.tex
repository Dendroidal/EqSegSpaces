...
\begin{abstract}
	We introduce the analogues of the notions of complete Segal spaces and of Segal categories in the context of equivariant operads with norm maps, and build model categories with these as the fibrant objects. We then show that these model categories are Quillen equivalent to each other and to the model category for 
...
was made clear by Hill, Hopkins and Ravenel, who constructed them in the context of $G$-ring spectra
...
Denoting the leftmost tree in the middle row of \eqref{HORN_EX_FIG} by $S$, the poset 
...
$S \to T \leftarrow -S$.
The argument then shows that $\Lambda_o^{Gb}[T] \to \Omega[T]$ is built cellularly by attaching
...
By specifying to the cases of $T = G \cdot T_{\**}$
the free $G$-trees,
...
\begin{example}\label{GGRAPHREEDY EX}
	...
	$K \leq G \times \mathsf{Aut}_{\mathbb{S}}(s)$ 
	which are graphs of partial homomorphisms
	$G \geq H \to \mathsf{Aut}_{\mathbb{S}}(s)$. We note that $G$-graph subgroups are also characterized by the condition $K \cap \mathsf{Aut}_{\mathbb{S}}(s) = \{e\}$.
...
In fact, in this case we are also interested 
...
in Definition \ref{GENRED DEF}, and is the actual novelty in this appendix.
...
In the following, given $K \leq \mathsf{Aut}(\mathbb{S})$,
we write $K \ltimes \mathbb{S} \to K$ for the Grothendieck construction. Informally, 
$K \ltimes \mathbb{S}$ is obtained from $\mathbb{S}$
by formally adding arrows $h \colon s \to hs$.
...
Since the cofibration half of the projective analogue of Corollary \ref{RESGEN COR} only holds if $\phi$ is a monomorphism, the argument in the proof of \cite[Lemma 5.3]{BM11} also includes an injectivity check that is not needed in our proof of Lemma \ref{REEDYTRCOF LEM}.
\end{remark}
