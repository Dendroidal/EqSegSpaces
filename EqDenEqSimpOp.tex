...
All horn inclusions attached in the proof of Lemma \ref{CHAREDGE LEM} correspond to $G$-trees whose non-equivariant components are faces of the $U_i$.
\end{remark}
...
\begin{proof}
	...
	$U_{F'}=T-F'$ therein satisfy $|T-F'|<|T|$.
\end{proof}
...
\begin{proof}
	The fact that $G$-inner horn inclusions generate orbital $G$-inner horn inclusions and $G$-Segal core inclusions was already established in Proposition \ref{ORB_HORN_PROP} and Remark \ref{RECOVER REM}(i).
	
	To see that all $G$-inner horn inclusions are in the hypersaturation of the orbital horns, we again argue by induction on $|T_{\**}|$. Recalling that in the proof of the $E = \emptyset$ case of Proposition \ref{ORB_HORN_PROP} it is 
	$I=\**$, $U_{\**} = T$ and $\Xi^{\**} = F$,
and noting that the maximum of $\mathsf{Face}_{F}^{lex}(T)$
is $T$, one has a factorization
	$\Lambda_o^F[T] \to \Lambda^F[T] \to \Omega[T]$
	such that the induction hypothesis applies to show that 
	$\Lambda_o^F[T] \to \Lambda^F[T]$ is in the hypersaturation of orbital horns.
