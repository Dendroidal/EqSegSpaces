...
{\color{red} HERE}
=======
        as in general this would lead to repeated elements in $[e]^{\uparrow}$.
...
%		\draw[->]
%	(T2)++(0.8,.5) -- (-1,0)+(S) node [midway,above] {$+1$};
		\end{scope}%
%----Orbital picture-------
...
below since the bottom map is inner while the top and left maps are given by the center and right pullback diagrams.
\begin{equation}
\begin{tikzcd}
...
\begin{equation}
      ...
      (G \cdot j)/N: G \cdot \left(
            {\Lambda^{E_*}[T_*] \to \Omega[T_*]}
      \right) / N,
\end{equation}
where $E_* = E \cap T_*$, $H = \mathrm{Stab}_G(T_*)$, and
$j$ is a generalized inner horn inclusion in $\Omega$ (cf. \eqref{T_DECOMP_EQ}). 
...
\begin{proof}
      ...
      we have the following commuting diagram, where $n \in N$.
      \begin{equation}
            \begin{tikzcd}[row sep = 3em]
                  G \cdot \Lambda^{H e}[T_*] \arrow[d] \arrow[r, "n"]
                  &
                  G \cdot \Lambda^{H e}[T_*] \arrow[r, "q"] \arrow[d]
                  &
                  \Lambda^{G e}[T] \arrow[r, "f"] \arrow[d]
                  &
                  X
                  \\
                  G \cdot \Omega[T_*] \arrow[r, "n"']
                  \arrow[urrr, very near start, crossing over, dashed, "\exists! \phi"]
                  &
                  G \cdot \Omega[T_*] \arrow[r, "q"']
                  \arrow[urr, dashed, very near start, crossing over, out=15, in=-145, "\exists! \phi"]
                  &
                  \Omega[T]
            \end{tikzcd}
      ...
      Second, we know $G \cdot (-)$ commutes with $(-)^{\boxslash !}$, and
      % \begin{equation}
