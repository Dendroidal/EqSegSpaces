...
\begin{example}
	...
	and consider the tree $T \in \Omega^G \subset \Omega_G$ at the top of \eqref{HORN_EX_FIG} below.
...
By specifying to the cases of free $G$-trees $T = G \cdot T_{\**}$,
...
\begin{remark}
Historically, the joint Reedy model structure has been called the \textit{Rezk model structure} with fibrant objects the 
...
\begin{remark}\label{CONCRECOM REM}
      ...
      by the object $X^{J^{\bullet}} \in \mathsf{sdSet}^G$,
...
\]
We note that this holds even if $C = C_{\varnothing}$ is a 0-corolla, %$k=0$, 
...
(note that by the sieve condition (i),
