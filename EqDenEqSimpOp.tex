...
\begin{equation}\label{ORBHORNINC EQ}
	\Lambda_o^{E \amalg F}[T] \to \Lambda_o^{F}[T]
\end{equation}
	are $G$-inner anodyne.
\end{proposition}
...
$G \cdot_H 
\left( \Lambda_o^{E_{\**} \amalg F_{\**}}[T_{\**}] \to \Lambda_o^{F_{\**}}[T_{\**}]\right)$.

Further, we are clearly free to assume $E \neq \emptyset$.
There are two main cases, 
$F = \emptyset$ and $F \neq \emptyset$.

If $F=\emptyset$ then $\Lambda_o^{F}[T] = \Omega[T]$ and we apply
...
It remains to check the characteristic conditions in Definition \ref{CHAREDGE DEF}.
	...
	$GV = T-E'$ for some $G$-equivariant subset
	$E' \subseteq E$.

	For (Ch1), the condition $\Xi_{V} = \emptyset$
	says that none of the inner edges of $V$ are in $E$,
	and thus that the equivariant outer face $G V$ contains none of the edge orbits in $E$ as inner edge orbits. Since $E \neq \emptyset$, the equivariant outer face $GV$ is not $T$ itself, 
	...
	In the case $F \neq \emptyset$ we instead apply Lemma \ref{CHAREDGE LEM} with $I = E/G$, with an 
	\textit{arbitrary} choice of total order, and 
	(writing elements of $E/G$ as orbits $Ge \subseteq E$)
\[
	\Xi^{Ge} = F, \qquad 
	U_{G e}= T - Ge, \qquad
	X=\Lambda_o^{E\amalg F}[T].
\]
Note that the $U_{Ge}$ are the orbital inner faces $T - Ge$ for $Ge \subseteq E$, and thus the map
...
We again check the characteristic conditions. (Ch0) is clear.
	...
	$GV = T - E'-F'$ and $G(V - \Xi_V) = T-E'-F''$ with
	$E' \subseteq E$, $F' \subseteq F'' \subseteq F$,
...
Inclusions of $G$-inner horns
\begin{equation}
	\Lambda^{E \amalg F}[T] \to \Lambda^{F}[T]
\end{equation}
are $G$-inner anodyne.
\end{proposition}

\begin{proof}
The case $F = \emptyset$ is now tautological,
hence we can now assume that both $E,F \neq \emptyset$.

We now apply Lemma \ref{CHAREDGE LEM} with 
$I = \mathcal{P}_0(E)$
the poset of non-empty subsets $\emptyset \neq E' \subseteq E$, ordered by \textit{reverse inclusion}, and
\[
	\Xi^{E'} = F, \qquad
	U_{E'} = T - E', \qquad
...
\begin{remark}\label{TWOPROOF REM}
	...
	$\Lambda^{E \amalg F}[T] \to \Lambda^{F}[T]$
	are inner anodyne,
	with the first proof using $I = E$ (with any order) and a second using 
	$I = \mathcal{P}_0(E)$. 
	...
	``intersections'' of $T-e$ and $T-ge$ for $e \in E$. 
\end{remark}
...
$G$-equivariant $E \subseteq E^{\mathsf{i}}(T)$; 
...
In the following proof we call the hypersaturation
of the orbital horn (resp. Segal core) inclusions the orbital (resp. Segal) hypersaturation.
...
\begin{proof}
	...
	To see that the $G$-inner horn inclusions are in the orbital hypersaturation, we again argue by induction on $|T_{\**}|$. Recalling that in the proof of the $F = \emptyset$ case of Proposition \ref{ORB_HORN_PROP}
	one sets
	$I=\**$, $U_{\**} = T$ and $\Xi^{\**} = E$,
...
\end{remark}
\todo[inline]{define trivial fibrations in $\mathsf{sdSet}_G$.}
...
{\color{blue} The following connects the various model structures on $\mathsf{sdSet}^G$.
  \begin{definition}
        The \textit{equivariant complete Segal space model structure} on the category $\mathsf{sdSet}^G$
        is the left Bousfield localization of the equivarinat Segal space model structure
        with respect to the projections
        \begin{equation}
              \Omega[T] \otimes (\set{i} \to J_d), \qquad T \in \Omega_G.
        \end{equation}
  \end{definition}

  \begin{proposition}
        The joint Reedy model structure and the complete Segal space model structure are equal.
  \end{proposition}
  \begin{proof}
        As these two model structures have the same class of cofibrations,
        it suffices to show they have the same fibrant objects,
        which follows by Corollary \ref{JOINTFIBCHAR COR}(iii).
  \end{proof}

  In what follows, we will use ``complete Segal space'' and ``joint fibrant'' interchangably.
}
...
on $X$ (defined exactly as in Definition \ref{PROF DEF}\footnote{
  Recall that $\mathsf{sdSet}^G(\partial\Omega[C], X) = \mathsf{dSet}(\partial\Omega[C], X_0) = \mathsf{dSet}(\partial\Omega[C], X^{\Delta[0]} = \mathsf{sSet}(\Delta[0], X^{\partial\Omega[C]})$.
  })
...
\end{tikzcd}
\]
\todo[inline]{this time, the bottom right might work, but I don't know why $X^{\partial\Omega[C]}$ wouldn't be better}
\end{definition}

\begin{definition}
	Let $X \in \mathsf{sdSet}^G$ be a dendroidal Segal space.
	The \textit{homotopy genuine operad} 
	$ho(X)\in \mathsf{dSet_G}$ is defined %
...
\begin{remark}
	...
	\Omega_G$,
	one has that $\iota_G^{\**}ho(X)$ is the $G$-coefficient system of categories\todo{simplicial sets? we haven't shown they have composition yet} formed by the homotopy categories
	$ho \left(\iota^{\**}\left(X^H\right)\right) = 
	\pi_0 \left(\iota^{\**} \gamma_{\**}X^H\right)$.
        \todo[inline]{is $ho$ of a bisimplicial set well-defined? is this $ho$ of the diagonal?}
        \todo[inline]{the RHS is only a simplicial set.}
        Indeed, we have that each $X^H$ is a dendroidal Segal space if $X$ is
        (as $(X^H)^{\Omega[T_*]} \to (X^H)^{Sc[T_*]}$ has the right lifting property against all monomorphisms in $\mathsf{sSet}$ iff $X^{\Omega[G \cdot_H T_*]} \to X^{Sc[G \cdot_H T_*]}$ does).
\end{remark}
...
and the fact that by the previous corollary \todo[inline]{there is no previous corollary} the maps
