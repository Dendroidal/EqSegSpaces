...
\begin{example}
      ...
      We note that for this tree, the poset $\mathsf{Face}_o(T) \cap \Omega^G$
      (which includes all maximal orbital faces)
      is isomorphic to a subposet of $\mathsf{Face}(T)$,
      and hence the orbital horn $\Lambda^{G b}_o[T]$ is determined by
      the convex subset of $\mathsf{Face}(T)$ generated by
      $(\mathsf{Face}_o(T) \cap \Omega^G) \setminus (T - Gb)$.
      The \textit{compliment} of this poset,
      determining $\Omega[T] \setminus \Lambda^{G b}_o[T]$,
      is displayed in \eqref{HORN_EX_FIG} below.
      Furthermore, $\Lambda^{G b}[T]$ is just missing the four boxed faces,
      and thus the four remaining faces determine $\Lambda^{G b}[T] \setminus \Lambda^{G b}_o[T]$. 
      ...
      \begin{equation}\label{HORN_EX_FIG}
            ...
            \begin{tikzpicture}[grow= up, level distance = \mywidth, auto,
                  ...
                  \draw[->]
                  (Pab)++(1.1,1.8) -- ($(Tbb)+(-.2,-.2)$); % ++(.5,.3\rh);
                  \draw[->]
                  (Pabb)++(-1.1,1.8) -- ($(Tb)+(.2,-.2)$); % ++ (-.5,.3\rh);
                  \draw[->]
