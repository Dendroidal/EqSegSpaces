...
\begin{definition}
      ...
      \todo[inline]{the colimit wasn't functorial as written: e.g. if $U_0$ is an equivariant subface of $S_0$, then $\Omega[G \cdot_K U_0]$ does not canonically map into $\Omega[G \cdot_L S_0]$. This can be fixed by making the poset larger (adding full orbits of certain subface subinclusions)}
  \end{definition}
...
\begin{proof}
      ...
      Conversely, by \cref{SRLP_IHI_THM} and \cite[Theorem 6.1, Proposition 6.10]{MW08}, we have that
...
\begin{proposition} Inclusions of orbital horns
...
\]
Note that the $U_i$ are precisely the orbital inner faces $T - Gf$ for $Gf \subseteq F$, and thus the map
in Lemma \ref{CHAREDGE LEM} is indeed \eqref{ORBHORNINC EQ}.
We must again check the characteristic conditions. (Ch0) is clear.
	...
	Lastly, we show (Ch3).
	Again by	
	Remark \ref{GINNER REM} it is
	$G(V-\Xi_V) = GV - E'$ for $E' \subseteq E$ and 
	thus $(V-\Xi_V) \hookrightarrow U_j$ iff
	$V \hookrightarrow U_j$. 
	This suffices for (Ch3) regardless of the chosen well-ordering.
\end{proof}
