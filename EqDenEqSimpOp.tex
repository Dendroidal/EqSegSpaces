...
It was further shown that $N_{hc}(\O)$ is such an object
% has this property
for any suitably fibrant $G$-operad $\O \in \mathsf{sOp}^G$
(as opposed to the purely formal conclusion being an ``$\infty$-operad with $G$-action'').
...
It is worth noting that, much as was the case with the work in \cite{Per17}, our results are not formal consequences of their non-equivariant analogues, due to the nature of norm maps\footnote{Recall that by using the inclusions of simplicial categories and simplicial sets into simplicial operads and dendroidal sets, the Cisinski-Moerdijk program recovers the Bergner-Joyal-Lurie-Rezk-Tierney program studying $\infty$-categories. As a point of contrast, we note that the lack of norms in the categorical case causes the equivariant generalization of this latter program to indeed be formal; see \cite{Ste16,Ber17}.}.
...
there is % we have %one can obtain
an associated broad poset just as before, and thus a category $\Phi$ of forests.
Letting $\Phi^G$ denote $G$-objects on $\Phi$, referred to as $G$-forests,
% Letting $\Phi^G$, the category of $G$-forests, denote $G$-objects on $\Phi$,
the category
...
On the one hand, the $G$-broad posets associated to $G$-trees are diagrammatically represented by the expanded representation\footnote{
  The orbital representation for $T$ is the diagram for the \textit{quotient} broad poset $T/G$.},
...
\begin{remark}
      ...
      \end{equation}
      where $T/G$ is the quotient broad poset with associated tree diagram given by the orbital representation.
\end{remark}
...
\begin{remark}
      \label{ORB_HORN_REM}
      Furthering the observation from Remark \ref{ORB_FACE_REM} that $\mathsf{Face}_o(T) \simeq \mathsf{Face}(T/G)$,
      we record that this isomorphism induces an isomorphism of posets of subcomplexes
      \begin{equation}
            \sets{\bigcup_i\Omega[S_i] \subseteq \Omega[T]}{i \in \NN,\ S_i \in \mathsf{Face}_o(T)} \simeq
            \sets{\bigcup_i\Omega[U_i] \subseteq \Omega[T/G]}{i \in \NN,\ S_i/G \in \mathsf{Face}(T/G)}.
      \end{equation}
      In particular, $\Lambda^{Ge}_o[T]$ corresponds to $\Lambda^{[e]}[T/G]$.
\end{remark}
...
=======
Following the discussion preceding \cite[Prop. 3.6.8]{HHM16}, we say
>>>>>>> master
...
so $\dSet_h^{\boxslash}$ and $(\dSet^{G,h})^\boxslash$ are the categories of $(G)$-$\infty$-operads.
...
\begin{proof}
      ...
      \begin{equation}
            \begin{tikzcd}
                  ...
                  &&
                  \iota_!\Lambda^{H.e}[T_*]/N \simeq \Lambda^{G e}[T] \arrow[dd, near end, "\iota_!j/N"] \arrow[r, "f"]
                  &
                  X
                  \\
                  &
      ...
      we have unique lifts $\phi$ as drawn by Lemma \ref{HYPER_LP_PROP} and Remark \ref{HORN_DECOMP_EQ}.
...
\begin{proof}
      ...
      Thus, \cite[Theorem 6.1, Proposition 6.10]{MW09} implies that
...
\begin{proof}
	...
	$(\gamma_{\**}X)(\Omega[T]) \to 
	(\gamma_{\**}X)(Sc[T])$
	is indeed a trivial Kan fibration.
\end{proof}
...
$X(x_1,\cdots,x_n ; x_0)$ via the pullback square
\[
\begin{tikzcd}[column sep=4em]
...
\begin{notation}
	Given a $C$-profile $(x_1,\cdots,x_k;x_0)$ and
	$\varphi \in X(x_1,\cdots,x_k;x_0)$
	we write 
\[[\varphi] \in ho(X)(C) =
	\pi_0 \left( \upsilon_{\**} \left( \gamma_{\**}X \right)(C) \right)=
	\pi_0 X\left(\Omega[C]\right)
\]
...
\begin{definition}\label{HEQUIV DEF}
	...
	\textit{$H$-equivalence} 
	if $[f]$ is an isomorphism in
	$ho\left(\iota^{\**}(X^H)\right)$.
\end{definition}

\begin{remark}
	It is immediate that
	$X(x_1,\cdots,x_k;x_0) = \gamma_{\**}X(x_1,\cdots,x_k;x_0)$, so that Definitions \ref{MAPSPACESEG DEF}, \ref{HMTPYGEN DEF}, \ref{DKEQUIV DEF} and  \ref{HEQUIV DEF}
	depend only on the fibrant pre-operads $\gamma_{\**}X, \gamma_{\**}Y$.
\end{remark}
...
(necessarily a map over $X\left(\partial \Omega[T-Ge]\right)$)
...
are well defined up to homotopy. 
	Further, if $[\varphi]=[\bar{\varphi}]$ then 
the maps $\varphi \circ_{Ge} (-)$, $\bar{\varphi} \circ_{Ge} (-)$
	are homotopic, and likewise for $[\psi] = [\bar{\psi}]$.
	In particular, $[\varphi \circ_{Ge} \psi]$
	depends only on $[\varphi]$, $[\psi]$.

	Lastly, the homotopy classes of the maps $\varphi \circ_{Ge} (-)$, $(-)\circ_{Ge} \psi$ are functorial with respect to maps $f\colon X \to Y$ between dendroidal Segal spaces.
\end{proposition}
...
\subsection{Completion and fibrant Segal operads}
