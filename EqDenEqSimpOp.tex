...
To recover \cite[Thm. 7.1(i)]{Per17} from Lemma \ref{CHAREDGE LEM}, we first let $I = \mathsf{Max}^{lex}(S \otimes T)$
...
\[%
	...
	\end{tikzpicture}%
\]%
\caption{The $\mathbb{Z}_{/2}$-poset $\mathsf{Max}^{lex}(S \otimes T)$ and characteristic edges $\Xi^i$}
\label{FIGURE}
\end{figure}
...
$\Xi^U_V = \emptyset$, 
then all edges of $V$ are in either
$S \otimes T_{\not < G\xi}$ or $S \otimes T^{\leq G\xi}$,
so that by \cite[Lemma 7.19]{Per17} it is either
...
Lastly, for (Ch2), suppose $V \hookrightarrow U_i$ and 
$(V - \Xi^{U_i}) \in X$.


bla bla lemma says factors iff it factors on edges, 

bla bla dismiss case $S \otimes \Lambda$ first.
...
\begin{proof}
      ...
      Moreover, fixing $\bar E \subseteq E$, %$\bar e \subseteq K e$,
...
\begin{proof}
      ...
      % \todo{replace $F_{Inn}(T)$ or $Inn(T)$ with $Inn_G(T)$? $E(T)$? $V(T)$? $V_G(T)$?}
      ...
      {\color{blue} % -------------------------COLOR: BLUE ---------------------------------------------------------
        ...
        \begin{equation}
              ...
              A_B
              % := A \cup \mathop{\bigcup}\limits_{B/G}\mathsf{im}\left(\Omega[G \cdot_{K} U_0]\right)
      ...
      % 
      %%%%%%% ------------- ARGUMENT FOR SOME OTHER PROOF? ------------------------------------------------------------
      % {\color{blue} % ---------------COLOR: BLUE -------------------------------------------------
      %   $H e$ is a characteristic edge orbit for $T_0$ with respect to $A$,
      %   where $e$ is any inner edge in $T_0$ with $T \simeq G \cdot_H T_0$ a decomposition for $T$.
      %   This follows by observing $\partial^{out}\Omega[T] \subseteq A$ and by applying Lemma \ref{FOF_OHORN_LEM}, using the fact that any inner face of $T_0$ is not contained in any outer face.
      %   } % ---------------------------------------------------------------------------------------------
      ...
      % (Ch 2) again follows from Lemma \ref{FOF_OHORN_LEM}.
\end{proof}
\begin{proof}[Proof Using More Advanced Characteristic Arguments]
      ...
      \end{equation}
      \todo[inline]{replace $\set{U_i}$ with the whole poset of faces}
      (Ch1) is immediate.
      ...
      \todo[inline]{deal with trivial cases: $|V_G| = 1$, etc}
\end{proof}
...
\begin{proof}
      ...
      By Proposition \ref{GHORN_IN_SC_PROP}\todo[inline]{fix this reference}, we see that
      \begin{equation}
            \label{SC_POUT_EQ}
