...
\begin{remark}
      ...
      then $\mathcal{C}^{\boxslash} = \mathcal{D}^{\boxslash}$. 
\end{remark}


\todo[inline]{COME BACK. Need to rework this}
...
We first recall the adjunction below.
\begin{equation}
      \begin{tikzcd}[column sep =5em]
      ...
      \end{tikzcd}
\end{equation}
We note that any $T \in \Omega_G$ can be written $(G \cdot T_{\**})/N$, where $T_{\**}$ is a comnponent of $T$ and
...
\begin{proof}
      ...
      we have unique lifts $\phi$ as written above by Lemma \ref{HYPER_LP_PROP} and Remark \ref{SKAN_REM}.
