      ...
      \begin{equation}
            \begin{tikzpicture}[grow= up, level distance = 1.75em, every node/.style = {font=\footnotesize}, auto]
                  ...
                  child{node [dummy] {}
                    ...
                    edge from parent node [swap] {$d+G/G$}
                  };
            \end{tikzpicture}
      \end{equation}
      We note that for this tree, the subposet of orbital faces with root orbit $G/G$
      (which includes all maximal faces)
      is isomorphic to a subposet of $\mathsf{Face}(T)$.
      Thus the orbital horn $\Lambda^{G b}_o[T]$ is determined by
      the convex subset of $\mathsf{Face}(T)$ generated by these faces, excepting $T-Gb$.
      % the convex subset of $\mathsf{Face}(T)$ generated by $\mathsf{Face}_{o,G}[T] \setminus \set{T-Gb}$.
      The \textit{compliment} of this poset,
      determining $\Omega[T] \setminus \Lambda^{G b}_o[T]$,
      is displayed (without the maximum element $T$) in \eqref{HORN_EX_FIG} below.
      Furthermore, $\Lambda^{G b}[T]$ is just missing the three faces on the left in \eqref{HORN_EX_FIG},
      and thus the four faces on the right determine $\Lambda^{G b}[T] \setminus \Lambda^{G b}_o[T]$. 
      % % To explore the orbital and usual horn $\Lambda^{Ge}_o[T]$ and $\Lambda^{Ge}[T]$ in this case,
      % Consider the subposet of $\mathsf{Face}(T)$ displayed in \eqref{HORN_EX_FIG} below.
      % The horn $\Lambda^{G b}[T]$ is only missing $T$ and the three faces
      % % $T-b$, $T-\set{-b}$, and $T-Gb$
      % on the left in \eqref{HORN_EX_FIG}.
      % Moreover,  % simply byy which elements of $\mathsf{Face}(T)$ each are missin%g.
      % Specifically, we see that $\Lambda^{G b}_o[T]$ additionally misses the four faces on the right of \eqref{HORN_EX_FIG}.
      \begin{equation}
            \label{HORN_EX_FIG}
            \newlength{\mywidth}
            ...
            \begin{tikzpicture}[grow= up, level distance = \mywidth, auto,
                  ...
                  % conjugates
                  \draw[<->]
                  (Tb)++(-1,.5) -- ++(-2,0) node [midway,above] {$+1$};
                  % faces
                  \draw[->]
                  (TGb)++(-1.1,1.8) -- ++(-.5,.3\rh);
                  \draw[->]
                  ...
                  % conjugates
                  \draw[<->]
                  (Pa)++(-1,.5) -- ++(-2,0) node [midway,above] {$+1$};
                  % mixed
                  \node (Pab) at (8,-\rh) {$\partial^{-a}-b$}
                  child{node [dummy] {}
                    child{node [dummy] {}
                  ...
                  \node (Pabb) at (12,-\rh) {$\partial^{a}-\set{-b}$}
                  child{node [dummy] {}
                    child{node [dummy] {}
                      child{edge from parent node [swap] {$-a$}}
                  ...
                  % conjugates
                  \draw[<->]
                  (Pabb)++(-1,.5) -- ++(-2,0) node [midway,above] {$+1$};
                  % faces
                  \draw[->]
                  (Pab)++(0,1.8) -- ++(0,.3\rh);
                  \draw[->]
                  ...
                  % hard faces
                  \draw[->]
                  (Pab)++(-1.1, 1.8) -- ++(-5.9, 0.3\rh);
                  \draw[->]
                  (Pabb)++(-1.1, 1.8) -- ++(-5.9, 0.3\rh);
            \end{tikzpicture}
      \end{equation}           
      Lastly, we have boxed the one orbital face displayed in the above poset.
...
Recall that categories can be identified with their nerves, as the functor
\begin{equation}
...
Similarly, \cite[Prop. 5.3 and Thm. 6.1]{MW09} show that the operadic nerve
\begin{equation}
      \label{NERVE_EQ}
...
In the equivariant setting, each $T \in \Omega_G$ generates a test $G$-operad,
and so the natural generalization of \eqref{NERVE_EQ}
lands not in $\dSet^G$ but
the richer presheaf category of
...
% \begin{remark}
%       Since $\upsilon^{\**}\colim_\alpha \upsilon_{\**}X_\alpha = \colim X_\alpha$ for all $X \in \dSet^G$,
%       Proposition \ref{HYPER PROP} and Lemma \ref{HYPER_LP_LEM} imply that
%       $X \in \dSet_G$ is a genuine operad iff
%       $X$ has the strict lifting property against images of all Segal core inclusions.
% \end{remark}
...
We end this section by discussing the failure of $\dSet^G$ to contain strict genuine operads.
Indeed, a first guess for such objects would be the direct analogue of \cite[Prop. 5.3 and Theorem 6.1]{MW09},
