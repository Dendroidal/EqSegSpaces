...
We recall \textit{genuine equivariant operads} from \cite{BP17}.
Briefly, these are algebraic objects whose combinatorics is controlled by the category $\Omega_G$ of $G$-trees,
...
Non-equivariantly, the expanded representation encodes the composition map
...
yields that this map restricts to a composition of fixed points
\begin{equation}
      \label{NORM_COMP_EQ}
      \O(H/K)^H \times \O(K/L \amalg K/K)^K \to \O(H/L \amalg H/K)^H,
\end{equation}
where $\O(A)$ for $A \in \Set^H$ denotes $\O(|A|)$ with an appropriately twisted $H$-action\footnote{
  Equivalently, $\O(A)^H \simeq \O(|A)|^{\Gamma_A}$, where
  $\Gamma_A$ is a \textit{graph subgroup} of $G \times \Sigma_{|A|}$
  encoding the $H$-action on $A$.}.
...
these compatibilities describe how the norm maps encoded by $\P$ must interact under
restriction, conjugation, and composition.
For a formal definition, see \cite[\S4]{BP17}.

\begin{remark}
      Genuine operads are strictly more information than ``coefficient system of operads'':
      these latter objects are only indexed by the $\O(n)^H$, \footnote{
              Equivalently, only \textit{trivial} graph subgroup fixed points.}
        % are indexed analogously to the spaces $\O(A)^H$ (as opposed to simply the $\O(n)^H$)\footnote{
      and moreover the composition maps of genuine operads (e.g. \eqref{NORM_COMP_EQ})
      record the interactions of fixed points across different subgroups of $G$.
\end{remark}
...
\begin{example}
      ...
      For example, the \textit{commutative} genuine operad $\mathsf{Comm}(A) = \**$ encodes
      commutative monoids with all norm maps,
      while the \textit{naive commutative} genuine opeard
      \begin{equation}
...
We will need a combinatorial model for (weak) genuine equivariant operads.
This requires another important presheaf category, recalled in \eqref{DSETG_EQ} below.
First, we set notation.
\begin{notation}
      Given a class of maps $\mathcal C$ in a category, we say an object $X$ has the
      \textit{strict right lifting property} if the usual lifts exist and are unique.
\end{notation}
\cite[Prop. 5.3 and Theorem 6.1]{MW09} show that those $X \in \dSet$ with the strict right lifting property against
inner horn inclusions are precisely (the nerves of) operads.
Analogously, we find that those $X \in \dSet^G$ with the strict right lifting property against
$G$-inner horn inclusions are precisely (the nerves of) $G$-operads; see Proposition \ref{SRLP_OP_PROP} below.
Furthermore, for this result it is sufficient to only consider $G$-inner horn inclusions on $G$-free $G$-trees $T = G \cdot T_{\**}$,
leading to the conclusion that $\dSet^G$ is not rich enough to encode the full complexity of genuine equivariant operads.

Instead, we recall another important presheaf category in the discussion of equivariant trees,
...
\begin{remark}
      ...
      This further implies that $\Omega[T] = \upsilon^{\**} \Omega_G[T]$. 
\end{remark}
...
\begin{definition}
      ...
      $X$ has the strict right lifting property against the images of all $G$-inner horn inclusions under $\upsilon_{\**}$.
      \begin{equation}
            \label{GGIOP_EQ}
...
We end this section by proving the combinatorial characterization of $G$-operads mentioned above.
