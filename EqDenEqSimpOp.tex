...
\subsection{Equivariant trees and equivariant dendroidal sets}
...
But on the other hand, the diagrams encoding compositions of norm maps of an equivariant operad $\mathcal{O}$
are given by the orbital representations of the 
$G$-trees $T \in \Omega_G$ (see \cite[Ex. 4.9]{Per17}, \cite[(1.10)]{BP17}).
...
More explicitly, a $G$-profile is given by the data of: % consists of:
\begin{itemize}
...
\todo[inline]{come back: fill out this step}
\end{definition}
...
$k \left(G/H \cdot \eta, Z \right) = k(\iota^{\**}(Z^H))$, so that 
...
Assume now that $f$ is fully faithful and essentially surjective. Since \eqref{COMSQ EQ} is a homotopy pullback, Proposition \ref{TFAE PROP} now implies that one needs only check that the maps of Kan complexes
...
\end{tikzcd}
\end{equation}
is a weak equivalence. We note that the bottom map in 
\eqref{OMEGASQ EQ}
...
\begin{remark}
      ...
      we use $\square$ to denote the pushout-product under the ``external product'' $\mathsf{sSet} \times \mathsf{sSet} \xrightarrow{\times} \mathsf{ssSet}$;
      equivalently, we can first consider each map to be discrete in the other direction, and then take the normal box product in $\mathsf{ssSet}$. 
\end{remark}
