...
and that $N_{hc}(\O)$ for any suitably fibrant $G$-operad $\O$ has this structure.
...
Our official model for $\Omega$ will be Weiss' algebraic model of \textit{broad posets} as discussed in \cite[\S 5]{Per17} and \cite{Wei12},
...
Moreover, a face map is further called
a \textit{proper face map} if $\phi$ is not bijective,
an \textit{inner face map}
if $\varphi(r_S) = r_T$ and 
$\varphi(\underline{l}_S) = \underline{l}_T$ 
(where $r_{(-)}$ denotes the root edge and $\underline{l}_{(-)}$ the leaf tuple)
and an \textit{outer face map} if it does not factor through any non-identity inner face maps.
...
\begin{definition}
	...
	$U \hookrightarrow T$,
	and call them \textit{planar/proper/outer} faces
...
\begin{remark}
      For every component $S_i$ of an orbital face $S \into T$,
      the isotropy of $S_i$ equals to isotropy of the root $r(S_i)$.
      Moreover, for any (non-equivariant) face $U$ with this property, we have
      $HU = U$ and $GU = G \cdot_H U$.
\end{remark}
...
Lastly, we recall that there is another presheaf category important in the discussion of equivariant trees,
namely the category of \textit{genuine $G$-equivariant dendroidal sets} $\mathsf{dSet}_G = \mathsf{Set}^{\Omega_G^{op}}$.
The inclusion $\Omega \times G \into \Omega_G$ sending a tree $T$ to the $G$-free $G$-tree $G \cdot T$
induces a pair of natural adjunctions as below.
\begin{equation}
      \begin{tikzcd}[column sep =5em]
            \mathsf{dSet}_G \ar{r}[swap]{u^{\**}} 
            &
            \mathsf{dSet}^G
            \ar[bend right]{l}[swap,midway]{u_{!}}
            \ar[bend left]{l}{u_{\**}}
      \end{tikzcd}
\end{equation}

\begin{remark}
      We observe that, given $T \in \Omega_G$, there are two possible definitions for the notation $\Omega[T] \in \mathsf{dSet}_G$,
      namely the image of $T$ under the Yoneda embedding, and the image of $\Omega[T] \in \mathsf{dSet}^G$ under $u_{\**}$ .
      In fact, these operations agree.
      \begin{equation}
            \begin{tikzcd}
                  \Omega_G \arrow[dr, "Y"] \arrow[d, "{\Omega[-]}"']
                  \\
...
We will now prove that various maps in question are in fact $G$-inner anodyne.
...
and expanding on Definition \ref{SAT_DEF}, we say
...
We have some similar results about the saturation of the orbital horn inclusions.
...
\begin{proposition}
      ...
      \end{equation}
      can be built cellularly out of generating orbital horn inclusions.
\end{proposition}
\begin{proof}
...
\begin{proposition}
      ...
      For all $T \in \Omega_G$, $Sc[T] \to \Omega[T]$ is build cellularly out of orbital horn inclusions.
\end{proposition}
\begin{proof}
      ...
      \begin{equation}
            ...
            Sc[T] \cup \mathop{\bigcup}\limits_{B'}\Omega[S]
      \end{equation}
      is built cellularly out of orbital horn inclusions.
      ...
      $V \in Sc[T]$ implies
      $V \in Sc[S] \subseteq \partial^{out}_o\Omega[G \cdot_K U]$,
      ...
      \begin{equation}
            \partial^{out}_o\Omega[S] \to \Omega[S],
      \end{equation}
      and by Proposition \ref{GORB_OHORN_PROP}, the result is proven.
\end{proof}

\begin{remark}
      The above plus Proposition \ref{ORB_HORN_PROP} also implies that Segal core inclusions are $G$-inner anodyne.
\end{remark}
...
\begin{remark}
      We note that if $\mathcal{C}$ is a class of normal monomorphsism which are also weak equivalences, then
      the saturation and the hypersaturation of $\mathcal{C}$ are equal.
      Thus, if $\mathcal{D}$ is another such class such that the hypersaturations of $\mathcal{C}$ and $\mathcal{D}$ are equal,
      the proof above shows that $\mathcal{C}^{\boxslash} = \mathcal{D}^{\boxslash}$. 
\end{remark}
