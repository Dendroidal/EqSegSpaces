...
We recall that categories can be identified with their nerves, as the functor $N: \Cat \to \sSet$ is fully faithful.
Moreover, the essential image of the nerve is characterized as precisely those simplicial sets
with the strict\footnote{
  Recall that here \textit{strict} means that the usual lifts exist and are unique}
right lifting proprety against the inner horn inclusions \cite[Prop. 1.1.2.2]{Lur09}.
Similarly, \cite[Prop. 5.3 and Theorem 6.1]{MW09} show that the operadic nerve $N: \Op \to \dSet$ is fully faithful,
with image those dendroidal sets with the strict right lifting property against (dendroidal) inner horn inclusions.

\begin{remark}
      We note that these characterizations compliment the definitions for the weaker algebraic structures of
      $\infty$-categories and $\infty$-operads, by adding the adjective ``strict''.
\end{remark}
...
% The interactions between different lifting conditions (see \cite[Cor. 2.5, 2.6]{CM13a}, or \cite[Lemma 3.5 and Theorem 4.5]{JT06}
% \cite[Cor. 2.6]{CM13a}
% for example \cite[Cor. 2.6]{CM13a} shows that the essential image of the (dendroidal) nerve is
% also characterized by the strict right lifting property against (dendroidal) Segal core inclusions.
%  and \cite[Theorem 3.3]{JT06}
\begin{lemma}\label{HYPER_LP_LEM}
	If two classes $\mathcal{C},\mathcal{D}$
	of cofibrations in some model category %normal monomorphisms of $\mathsf{dSet}^G$
...
\end{proof}
\begin{remark}
      If $\mathcal C$ is a \textit{generating} set of trivial cofibrations,
      then the saturation and hypersaturations of $\mathcal C$ are both the class of trivial cofibrations,
      and so the above states that
      $\mathcal C$ and its hypersaturation have the same class of maps
      satisfying the (non-strict) right lifting property.
\end{remark}
...
Instead, we unpack Definition \ref{GEN_OP_DEF}. % recalling genuine operads (and the associated nerve functor).
Briefly, genuine equivariant operads from \cite{BP17} are algebraic objects
%controlled by $\Omega_G$, and
