...
\begin{proof}
	(i) simply reiterates the last part of Proposition \ref{COMBMODSTR PROP}.
	
	For (ii), note first that a simplicial fibrant $X$ will be joint fibrant iff it is local with respect to the dendroidal generating trivial cofibrations (\ref{DENDTRIVCOF EQ}), and by 
	Remark \ref{HYPERSATKAN REM}, this is equivalent to being local with respect to all maps
	\begin{equation}\label{DENDTRIVCOF2 EQ}
	\left(\Delta[0] \to \Delta[n]\right)
		\square
	\left(\partial \Omega[T] \to \Omega[T]\right),
	\qquad
	n\geq 0, T \in \Omega_G.
\end{equation}
The right lifting properties againt these maps say precisely that all vertex maps $X_n \to X_0$ are trivial fibrations in 
$\mathsf{dSet}^G$, hence the given condition is necessary.
...
\[
	\left(\partial \Delta [n] \to \Delta[n]\right)
		\square
	\left(\Lambda^{G e} \Omega[T] \to \Omega[T]\right),
	\qquad
	n\geq 0, T \in \Omega_G, e \in Inn(T)
\]
and 
\[
	\left(\partial \Delta [n] \to \Delta[n]\right)
