...
\begin{remark}\label{FACEGACT REM}
	...
	However, the $G$-isotropy $H\leq G$ of $U \in \mathsf{Face}(T)$ must not be confused with the isotropy of $\varphi_U$.
	...
	$\phi(h)$ is the leftmost isomorphism in
	(\ref{FACEGACT EQ}),
...
\begin{definition}\label{CHAREDGE DEF}
      ...
      \end{itemize}
      \todo[inline]{these are all \textit{proper} faces, yes?}
\end{definition}
...
\begin{proof}
We start with the case of $I \simeq G/H$ orbital {\color{blue} and discrete} so that, abbreviating $U = U_{[e]}$, $\{U_i\}$ is the set of conjugates $gU$. 
...
where here and in the remainder of the proof all outer closures are implicitly taken in $U$ (rather than $T$), i.e. 
$\bar{V}=\bar{V}^U$.
{\color{blue} In particular, we note that $V \into V'$ still implies $V < V'$.}
...
{\color{blue}
  For the ``only if'' direction of (b), let $W' = W - \Ksi_W$.
  ...
  if $\bar W = \bar V$, the definition of $\mathsf{Face}^{lex}_{\Ksi}(U)$ implies that $V$ must contain all of
  $\Ksi_V = \Ksi_{\bar V} = \Ksi_{\bar W} = \Ksi_{W}$ as inner edges,
  and thus not only $W - \Ksi_W = W' \into V$, but also $W \into V$.
}

We now show (c).
If $g(W-D)=W-D$ for some $D$ then $g(W - \Xi_W) \hookrightarrow U$,
...
$h(W-\Xi_W)=W-\Xi_W$\todo{here},
so that we may assume $D=\Xi_W$.
Now note that
$hW$, $h(W-\Xi_W)=W-\Xi_W$, and $W$
are all faces of $U$ with a common outer closure $\bar{W}$\todo{here}.
Thus
$h\Xi_{W} = \Xi_{hW} \subseteq \Xi_{\bar{W}} = \Xi_{W}$, where the last step follows since
$W \in \mathsf{Face}_{\Xi}^{lex}(U)$, and hence by cardinality reasons it must in fact be $h \Xi_{W} = \Xi_{W}$. But then $hW,W$
have the same outer closure and the same inner edges, and thus 
$hW=W$, establishing (c).
\todo[inline]{the two *here* above have a line of proof which is straightfoward set manipulation, but not shown}
...
is $G$-inner anodyne. 
Indeed, letting $I=\mathsf{Face}_{X'}^{out}(T)$ be the $G$-poset of outer faces $V \hookrightarrow T$, ordered by inclusion \todo{where is $X'$ in the definition?}, 
...
\todo[inline]{should $\Ksi^1$ include $r_1$ and $r_{-1}$?}
...
$\Ksi$ is independent of $G f$.
% this depends only on $V \hookrightarrow T$ and not on $Gf$.
...
\todo[inline]{or just $\set{U_{F'}}$ ordered as in $\mathsf{Face}(T)$}
\[
	\Xi^{F'} = E, \qquad
	U_{F'} = T - F', \qquad
...
\begin{example}
      ...
      and has maximal faces given by the top row below;
      the orbital horn $\Lambda^{G e}_o[T]$ has maximal faces $\partial^r$, $\partial^{G a}$, and $\partial^{G b}$,
      boxed below.
      We have also included some of the subfaces of $\partial^{a}$;
      those included in the orbital horn are (dashed) boxed.
      Note that $\partial^a$ and it's maxmimal subfaces outside the orbital horn
      themselves have faces both inside and outside the orbital horn.
      \begin{figure}[ht]
            \begin{equation}
