...
There is a history of analyses producing combinatorial models for algebraic objects,
by characterizing objects with the (strict) right lifting property against various classes of maps\footnote{
  Recall that here \textit{strict} means that the usual lifts exist and are unique}.
Classically, 
% \begin{enumerate}
% \item[(i)]
an $\infty$-category (resp. category) is a simplicial set with the (strict) right lifting property against the
inner horn inclusions (e.g. \cite{Lur09});
% \item[a(ii)]
more generally, an $\infty$-operad (resp. operad) is a dendroidal set with the (strict) right lifting property against the
dendroidal inner horn inclusions (\cite{MW09});
% \item[(iii)] (dendroidal) Segal spaces are simplicial spaces (resp. simplicial dendroidal sets) with the right lifting property against the
%       (dendroidal) Segal core inclusions (\cite{Rez01,CM13a}).
% \end{enumerate}

In this section, we will examine a ``strict'' equivariant generalization of the above results,
as we already have a model for \textit{weak} $G$-operads: those $X \in \dSet^G$ with the right lifting property against all
$G$-inner horn inclusions are exactly the $G$-$\infty$-operads of \cite{Per17}.

Integral to
the application of
% the larger context of
the above combinatorial structures is
how the various lifting properties interact and overlap;
for example, categories (operads) are also modeled by those presheaves with strict lifts against the Segal core inclusions
(\cite[Cor. 2.6]{CM13a}) (see also e.g. \cite{Ber10}).
% and the characterization of (dendroidal) Segal spaces requires that Segal core inclusions and inner horn inclusions have the same hypersaturation
% (\cite[Prop. 2.4, 2.5, 5.5]{CM13a}).
% \todo[inline]{delay the mention of Segal spaces until Section 5? Don't mention them at all?}
We begin with a general statement about such interactions.
...
Now, we will desire a combinatorial model for the \textit{genuine equivariant operads} from \cite{BP17}.
Briefly, genuine operads are algebraic objects,
inspired in part by the (weak) algebraic structure of $G$-$\infty$-operads,
whose combinatorics is controlled by the category $\Omega_G$ of $G$-trees.
Heuristically, they embody the fact that,
...
\begin{example}
      ...
      This  encodes the fact that the non-equivariant composition
      ...
      Here, $\O(A)$ for $A \in \Set^H$ denotes $\O(|A|)$ with an appropriately twisted $H$-action,
...
With this setup, genuine equivariant operads $\P \in \Op_G$ can be given by the data of
levels $\P(A)$ for each $A \in \Set^H$ (analogous to the evaulations $\O(H/K)^H$),
...
\begin{remark}
      Genuine operads are strictly more information than ``coefficient system of operads'',
      as these latter objects are controlled only by the subcategory $\amalg \Omega \times H^{op} \into \Omega_G$,
      and miss all of the interactions between fixed points across different subgroups of $G$.
\end{remark}
...
The natural first guess for combinatorial genuine operads would be
the equivariant analogue of \cite[Prop. 5.3 and Theorem 6.1]{MW09},
namely those $X \in \dSet^G$ with the \textit{strict} right lifting property against $G$-inner horn inclusions.
However, we find that $\dSet^G$ is too rigid, and the above encodes a more naive notion:
...
\begin{proof}
      ...
      Thus $N^G(\O)$ has the desired strict lifts % is in $(\mathsf{dSet}^{G,h})^{\boxslash !}$
      ...
      %$X \in (G \cdot \dSet_h)^{\boxslash !}$ iff 
...
that it is sufficient to only consider $G$-inner horn inclusions on $G$-free $G$-trees $T = G \cdot T_{\**}$;
that is, ``strictness'' reduces the involved combinatorics to simply $\Omega \times G^{op} \subset \Omega_G$.
To encode the full complexity of genuine equivariant operads,
we instead consider the richer presheaf category of
...
% sending a tree $U$ to the $G$-free $G$-tree $G \cdot U$.

\begin{remark}
      For $T \in \Omega_G$, the two ``Yoneda'' objects in $\mathsf{dSet}_G$ agree, as
      \mbox{$\Omega_G[T] = \upsilon_{\**}\Omega[T]$},
      implying in particular that $\dSet^G(\Omega[T],X) = \upsilon_{\**}X(T)$.
\end{remark}
...
\begin{remark}
      There is a model structure on $\dSet_G$ with fibrant objects those $X \in \dSet_G$ with the right lifting property against \eqref{GGIOP_EQ}
...
The two given models of genuine equivariant operads are in fact equivalent.
There is a natural map $N_G: \Op_G \to \dSet_G$ connecting genuine operads to their combinatorial backbone.
It then can be shown, using results analogous to \cite[Theorem 6.1]{MW09} or those in \S \ref{EDSS_SEC},
that for $X \in \dSet_G$,
$X$ is a (combinatorial) genuine equivariant operad iff $X \simeq N_G(\P)$ for some genuine operad $\P \in \Op_G$.
% \begin{equation}
%       \mbox{$X \simeq N_G(\P)$ for some $\P \in \Op_G$}
%       \qquad \mbox{iff} \qquad
%       \mbox{$X \in (\upsilon_{\**}\dSet^{G,h})^{\boxslash !}$,}
% \end{equation}
However, the proof of such a result requires the discussion of \textit{coloured} genuine equivariant operads,
and so we also delay this result to a sequel.
...
% We recall another important presheaf category in the discussion of equivariant trees,
...
%       \end{tikzcd}
% \end{equation}
% induced by the inclusion $\Omega \times G^{op} \into \Omega_G$ sending a tree $U$ to the $G$-free $G$-tree $G \cdot U$.
...
% As in \cite[Per17], we may consider combinatorial models for (weak) operads in $\dSet_G$.
% We recall that those $X \in \dSet$ with the (strict) right lifting property against
% inner horn inclusions are precisely the $\infty$-operads (resp. nerves of operads \cite[Prop. 5.3, Theorem 6.1]{MW09}).
...
% \begin{remark}
%       While the desiderata are clear, the precise model-theoretic framework is more subtle than in \cite{Per17},
%       and will be delayed to a sequel.
% \end{remark}
...
% Let us unpack this definition, and compare it with the already-defined genuine equivariant operads from \cite{BP17}.
...
% This has a natural equivariant analouge, as
% $(\dSet^{G,h})^{\boxslash}$ exactly describes the $G$-$\infty$-operads from \cite{Per17},
% while $(\dSet^{G,h})^{\boxslash !}$ are precisely the (the nerves of) $G$-operads:
...
% The proof of the ``if'' direction above shows in particular that
% the strict lifting property against $G$-inner horn inclusions depend only on \textit{free} $G$-trees,
% and so $(\dSet^{G,h})^{\boxslash !}$ is \textit{not} a good model for genuine equivariant operads.
% Indeed, the genuine equivariant operads from \cite{BP17} were designed as the
% algebraic objects with combinatorics controlled by the full range of $G$-trees,
% embodying the fact that while
% trees encode the combinatorics of composing generic operations,
% $G$-trees are built to encode the combinatorics of composing \textit{norm maps}.

% To illustrate, consider the $G$-tree from \eqref{TWOREP EQ} (or \cite[Example 4.9]{Per17}).
% Non-equivariantly, the expanded representation encdoes the composition map
% \mbox{$\O(2) \times \O(3) \times \O(3) \to \O(6)$}
% for any $G$-operad $\O$.
% However, keeping track of the $G$-action (or, equivalently, considering the orbital representation)
% implies that this map restricts to a composition of fixed points
% \begin{equation}
%       \label{NORM_COMP_EQ}
%       \O(H/K)^H \times \O(K/L \amalg K/K)^K \to \O(H/L \amalg H/K)^H,
% \end{equation}
% where $\O(A)$ for $A \in \Set^H$ denotes $\O(|A|)$ with an appropriately twisted $H$-action.
% Moreover, $\O(A)^H$ encodes norm maps $N^A X \to X$ for any $\O$-algebra $X$
% (with $N^A X$ defined as in e.g. \cite[\S6]{BH15}).

% With this setup, genuine equivariant operads $\P \in \Op_G$ are given by
% levels $\P(A)$ for each $A \in \Set^H$ (analogous to the evaulations $\O(H/K)^H$),
% composition maps of the form from \eqref{NORM_COMP_EQ},
% with appropriate naturality, unitality, and associativity conditions imposed by maps of $G$-trees.
% (for a formal definition, see \cite[\S4]{BP17}). 

% There is a natural map $N_G: \Op_G \to \dSet_G$ connecting genuine operads and their combinatorial backbone.
% It can be shown, using results analogous to \cite[Theorem 6.1]{MW09} or those in \S \ref{EDSS_SEC},
% that for $X \in \dSet_G$,
% \begin{equation}
%       \mbox{$X \simeq N_G(\P)$ for some $\P \in \Op_G$}
%       \qquad \mbox{iff} \qquad
...
% However, the proof of such a result requires \textit{coloured} genuine equivariant operads,
