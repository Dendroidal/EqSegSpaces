...
If $\{\Xi^i\}_{i \in I}$ is a characteristic inner edge collection of $\{U_i\}_{i\in I}$ with respect to $A$, then the map
	...
	\end{equation}
is $G$-inner anodyne. 
In fact, \eqref{CHARLEM EQ} is cellular\footnote{Given a class of maps $I$, an arrow $f$ is said to be \textit{cellular on $I$} or \textit{built cellularly from $I$} if
...
%	= \left(\iota^{\**}\left(X^{\Omega[T]}\right)\right)(1)
	...
	Since $X^{\Omega[T]}$
	is a (simplicial) Segal space (by adjunction together with \cite[Thm. 7.1]{Per18}),
