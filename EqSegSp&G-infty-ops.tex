...
\begin{abstract}
	We introduce the analogues of the notions of complete Segal spaces and Segal categories in the context of equivariant operads with norm maps, and build model categories with these as the fibrant objects. We then show that these model categories are Quillen equivalent to each other and to the model category for 
...
and 
$\mathsf{sOp}$ is the category of simplicial colored operads\footnote{
  These objects have also been referred to as \textit{multicategories enriched in simplicial sets}.
}.
...
The authors would like to thank the anonymous referee for their helpful suggestions and comments, including the creation and use of Table \ref{TABLE} from the introduction as an organizational summary.
...
If $\{\Xi^i\}_{i \in I}$ is a characteristic inner edge collection of $\{U_i\}_{i\in I}$ with respect to $A$, then the map
	...
	\end{equation}
is $G$-inner anodyne. 
In fact, \eqref{CHARLEM EQ} is cellular\footnote{Recall that a map $f$ is built cellularly from a class of maps $I$ if it can be obtained as a (possibly transfinite) composition of pushouts of maps in $I$ (but without retracts). See \cite[Def. 2.1.9]{Hov99},
...
\begin{proposition}
      ...
      \item $X(T) \to X(Sc[T])$ is a trivial Kan fibration for all $T \in \Omega_G$;
	\item $X(T) \to X(\Lambda^{Ge}[T])$ is a trivial Kan fibration for all $T \in \Omega_G$, $e \in \boldsymbol E^i(T)$;
      \item $X(T) \to X(\Lambda^E[T])$ is a trivial Kan fibration for all $T \in \Omega_G$, $G$-subsets $\varnothing \neq E \subseteq \boldsymbol E^i(T)$;
      \item $X(T) \to X(\Lambda_o^E[T])$ is a trivial Kan fibration for all $T \in \Omega_G$, $G$-subsets $\varnothing \neq E \subseteq \boldsymbol E^i(T)$.
      \end{enumerate}
\end{proposition}
