...
This remark readily implies the following.

\begin{proposition}\label{DKCOM PROP} 
Let $f\colon X \to Y$ in $\mathsf{sdSet}_G$ be a map between dendroidal Segal spaces. Then:
\begin{enumerate}[label=(\roman*)]
\item if $f$ is a DK-equivalence and the maps $X(\eta)^H \to Y(\eta)^H, H\leq G$ are Kan equivalences then $f$ is a simplicial equivalence;
\item if $f$ is a simplicial equivalence then $f$ is a DK-equivalence.
\end{enumerate}
\end{proposition}
...
complete/joint equivalences coincide with simplicial equivalences. But then by Proposition \ref{DKCOM PROP}
