...
In fact, we will prefer to use the common localization perspective to define (cf. Definition \ref{JOINREED DEF} and Remark \ref{JOINREED REM}) the equivariant Rezk/complete model structure on $\mathsf{sdSet}^G$,
...
a charaterization for complete equivariant dendroidal Segal spaces in Proposition \ref{JOINTFIBCHAR PROP}
...
as one of these ``joint left Bousfield localizations'', though our localizations are organized differently \footnote{As a result of this choice, the direct analogue of \cite[Thm. 6.6]{CM13a}, found in Remark \ref{RECOVDEF REM}, does not form an essential or even necessary piece of our analysis.}.
%
...
$\underline{\mathsf{Hom}}(A,X)_{\bullet} = \mathsf{Hom}(\Delta[\bullet] \times A,X)$.
...
% i.e. to the loop spaces of $X(\eta)^H$ at $x$.
