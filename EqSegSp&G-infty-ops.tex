...
\begin{inparaenum}
	...
	\item[(iii)] lastly, \cite{CM13b} established the existence of the model structure on $\mathsf{sOp}$ as well as the Quillen equivalence between $\mathsf{sOp}$ and $\mathsf{PreOp}$, finishing the proof of the main result of the program.\footnote{This narrative is itself a generalization of the interactions between the various models for $(\infty,1)$-categories, including complete Segal spaces and Segal categories. A survey of such models can be found at e.g. \cite{Ber10}.}
\end{inparaenum}
...
The following table provides a summary of the parallel narratives, listing the different model categories as well as the nomenclature for the fibrant objects.
\begin{table}[htbp]
      \centering
      \begin{tabular}{|c|c|c|}
        \hline
        ``categories up to htpy'' & ``operads up to htpy'' & ``genuine equiv. operads up to htpy''
        \\ \hline
        simplicial sets $\sSet$ & dendroidal sets $\dSet$ & equivariant dendroidal sets $\dSet^G$
        \\
        Joyal model structure & model structure from \cite{CM11} & model structure from \cite{Per18}
        \\
        $\infty$-categories & $\infty$-operads & $G$-$\infty$-operads
        \\ \hline
        bisimplicial sets $\mathsf{ssSet}$ & simplicial dendroidal sets $\mathsf{sdSet}$ & equiv. simplicial dend. sets $\mathsf{sdSet}^G$
        \\
        Rezk model structure & model structure from \cite{CM13a} & model structure from \S \ref{CEDSS SEC}
        \\
        complete Segal spaces & complete dendroidal Segal spaces & complete equiv. dend. Segal spaces
        \\ \hline
        Segal precategories $\mathsf{SeCat}$ & Segal preoperads $\mathsf{PreOp}$ & equiv. Segal preoperads $\mathsf{PreOp}^G$
        \\
        Hirschowitz-Simpson & model structure from \cite{CM13a} & model structure from \S \ref{PREOP SEC}
        \\
        ...
        \\
        Bergner model structure & model structure from \cite{CM13b} & model structure from \cite{BP_sq}
        \\ \hline
      \end{tabular}
\end{table}
...
and our narrative has been modified to highlight this.
...
As such, we will find it helpful to first focus on the abstract properties of such ``joint left Bousfield localizations''\footnote{
  The equivariant analgoue of \cite[Thm. 6.6]{CM13a} can be found in Remark \ref{INTERP REM}, and will be a consequence of this abstract analysis and an understanding of the fibrant objects in question.}.
...
\begin{remark}
      When $G = e$ and $C = [1]$, \eqref{OTIMESOC_EQ} recovers the decomposition of the product of simplicial sets
      \[
...
\begin{remark}\label{INTERP REM}
      ...
      (regarded a constant in the simplicial direction),
...
we write $K \ltimes \mathbb{R} \to K$ for the Grothendieck fibration associated to the action map $K \to \Cat$.
Informally, we have added arrows $k \colon r \to kr$ for all $k \in K$ and $r \in \mathbb R$, such that
