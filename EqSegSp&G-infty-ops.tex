...
as dually as possible, but some care is needed; see Remarks \ref{COFNOTRES REM}, \ref{NOTDUAL REM}).
%  and \ref{NOTDUAL REM}
%).
...
$A_{(-)} \amalg_{L_{(-)}A} L_{(-)}B
\to B_{(-)}$,
one has $l_{(r,s)} f \simeq l_r l_s f \simeq l_s l_r f$ in the arrow category
