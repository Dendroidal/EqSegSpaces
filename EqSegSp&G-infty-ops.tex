...
and 
$\mathsf{sOp}$ is the category of simplicial colored operads\footnote{These objects have also been referred to as \textit{multicategories enriched in simplicial sets}.
}.
...
\begin{inparaenum}
	...
	\item[(iii)] lastly, \cite{CM13b} established the existence of the model structure on $\mathsf{sOp}$ as well as the Quillen equivalence between $\mathsf{sOp}$ and $\mathsf{PreOp}$, finishing the proof of the main result of the program\footnote{Recall that by using the inclusions of simplicial categories and simplicial sets into simplicial operads and dendroidal sets (cf. the introduction to \cite{CM13b}), the Cisinski-Moerdijk program recovers and is itself a generalization of the Bergner-Joyal-Lurie-Rezk-Tierney program studying the various models for $(\infty,1)$-categories.
...
Rather than aiming for a direct equivariant analogue\footnote{Such a result can be found in Corollary \ref{JOINTCSS_COR}.},
...
%
As such, in \S \ref{JOINBOUS SEC} we first discuss an abstract setting for such common or \textit{joint} localizations,
as well as characterizations of the ``joint fibrant'' objects and equivalences in Proposition \ref{COMBMODSTR PROP}.
This is then applied in \S \ref{CEDSS SEC} to obtain
a charaterization for complete equivariant dendroidal Segal spaces in Corollary \ref{JOINTFIBCHAR COR}
and the Quillen equivalence
...
\begin{remark}
      The category $\Omega$ generalizes the simplicial category $\Delta$.
      Specifically, we have an inclusion $\Delta \into \Omega$ sending $[n]$ to the linear tree with $n+1$ edges,
...
$Y(U) \setminus X(U)$ has an $\mathsf{Aut}(U)$-free action for all $U \in \Omega$ (non-equivariantly, this is \cite[Prop. 1.5]{CM11}, and also holds equivariantly by \cite[Rem. 6.7]{Per18}; alternatively, it can be shown using \cite[Prop. 6.5(ii)]{Per18} and \cite[Prop. 5.62]{Per18}).
...
\begin{lemma}\label{CHAREDGE LEM}
      ...
      \end{equation}
      is $G$-inner anodyne. 
      In fact, \eqref{CHARLEM EQ} is built cellularly \footnote{Given a class of maps $I$, an arrow $f$ is said to be \textit{built cellular from $I$} if
        $f$ can be constructed out of transfinite composites of pushouts of maps in $I$
        (Hovey calls these \textit{relative $I$-cell complexes} \cite[Defn. 2.1.9]{Hov99}).
      % ; see also \cite[Cor. 2.1.15]{Hov99}, \cite[Example 12.6.12]{Ri14}).
...
(indeed, \cite[Lemma 7.37]{Per18} makes the claim that such a relation can be performed on an ``elementary subtree'' \cite[Def. 7.16]{Per18} or, in our notation, on the outer closure $\bar{V}^{U_i}$; via grafting, it is straightforward to extend such a $<$ relation for $\bar{V}^{U_i}$ to one for $U_i$ \cite[Rem. 7.30]{Per18}). But then, as one sees from \eqref{GENLEXREL EQ},
...
the fine (or genuine) equivalences, which care about all fixed point spaces, and the coarse (or na\:ive) equivalences, 
...
as one of these ``joint left Bousfield localizations'', though our localizations are organized differently \footnote{As a result of this choice, the direct analogue of \cite[Thm. 6.6]{CM13a}, Corollary \ref{JOINTCSS_COR}, does not form an essential or even necessary piece of our analysis.}.
%
...
\begin{corollary}[{cf. \cite[Thm. 6.6]{CM13a}}]
      \label{JOINTCSS_COR}
      The joint model structure is the localization of the dendroidal Reedy model structure with respect to all maps in $\mathsf{sdSet}^G$ of the form
      \[
            Sc[T] \longto \Omega[T],
            \qquad
            \Omega[T] \otimes (\{i\} \longto J),
      \]
      for $T \in \Omega_G$, with all objects simplicially constant.
\end{corollary}
\begin{proof}
      This is immediate from Corollary \ref{JOINTFIBCHAR COR}, as for all $A \in \dSet^G \into \mathsf{sdSet}^G$ and $X \in \mathsf{sdSet}^G$,
      the space of maps in $\mathsf{sdSet}^G$ from $A$ to $X$ is $X(A)$.
\end{proof}
...
\begin{remark}
      ...
      by first introducing the intermediate and more intuitive model structure for Segal spaces,
      and delaying the discussion of completeness.
      % 
...
\begin{remark}
	...
	\Omega_G$,
        unpacking definitions shows that $\iota_G^{\**}ho(X)$ is the $G$-coefficient system of simplicial sets with $m$-simplicies
