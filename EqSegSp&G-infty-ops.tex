...
the map $X \to Y$ is fully faithful iff \eqref{DKCOM EQ} 
induces weak equivalences between the fibers of the vertical composite maps (for the ``if'' direction, consider the case of $T$ a $G$-corolla).
...
For \eqref{LIFTCSK EQ}, Remark \ref{HYPERSATKAN REM} and 
...
the map from 
$X^{J^m}(\Omega[T])$
to the pullback of the other terms is a trivial Kan fibration. 
The Kan fibration claim is already known (from the lifting condition against \eqref{LIFTBASIC EQ}), while the weak equivalence claim follows since, by Remarks \ref{ALLXJK REM} and \ref{DKCOM EQ}, the diagram 
\eqref{MOREMORE EQ} induces Kan equivalences between the fibers of its vertical maps.
 
We can now finally establish that 
$\tilde{X} = \delta^{\**} \left(X^{J^{\bullet}} \right)$
is dendroidal Reedy fibrant. 
Indeed, since
$\left(\tilde{X}(\Omega[T]) \to \tilde{X}(\partial\Omega[T])\right) = 
\delta^{\**} \left( X^{J^{\bullet}}(\Omega[T]) \to X^{J^{\bullet}}(\partial \Omega[T])\right)$,
the fact that this is a Kan fibration when $T= G/H \cdot \eta$
follows from the lifting conditions against \eqref{LIFTBASIC EQ},\eqref{LIFTETA EQ}
and the second ``moreover'' condition in Corollary \ref{SSETSSETADJ COR} while when $T \neq G/H \cdot \eta$ it follows from the lifting conditions against \eqref{LIFTBASIC EQ},\eqref{LIFTCSK EQ} and the first ``moreover'' condition in Corollary \ref{SSETSSETADJ COR}.
...
and fully faithfullness now follows from Proposition \ref{SSSETJREE PROP}(iv) applied to these fibers.
...
\begin{theorem}\label{COMPIFFDK THM}
	A map $X \to Y$ of dendroidal Segal spaces is a complete/joint equivalence iff it is a DK-equivalence.
\end{theorem}


\begin{proof}
By Proposition \ref{COMPLE PROP}(i) one is free to assume that $X,Y$ are complete/joint fibrant, so that by Proposition \ref{COMBMODSTR PROP}(ii)
complete/joint equivalences coincide with simplicial equivalences.
