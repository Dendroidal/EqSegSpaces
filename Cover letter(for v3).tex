...
\item[37.] I do not think the proof of Proposition \ref{HYPER PROP} should require anything analogous to the mentioned condition (b) in the proof of Lemma \ref{CHAREDGE LEM}.
      The main idea is that for $U \in \Omega$ and $V \hookrightarrow U$ a planar face, it is $|V|<|U|$ unless $V=U$. We added an extra sentence to Remark \ref{FACCES REM} to clarify this point. % that should make this point clearer. %
      

\item[53.] The adjunctions from Notation \ref{UNIQUELIM NOT} are only used in simple cases, so we only briefly give the necessary equivalence of lifting problems and postpone the fuller discussion to 
%
%from this notation are only used in a single instance, in the proof of %part (i) of Proposition \ref{SSSETJREE PROP}; this use is also simpler since only the single $\partial \Delta[m]$ isn't a representable.
...
(the new) Remark \ref{TWOVARADJ REM}. %
...
\item[30.] Corrected $A_C = 
      ...
      Expanded Remark \ref{FACEGACT REM} to explain why $\Omega[gV] = g\Omega[V]$ and better explain the connection with isotropy of representable subpresheaves. % 
     
%      Added the alternate $g \Omega[V]$ formula already in the proof of the characteristic edge lemma.
...
\item Added Remark \ref{DUMBHYPER REM} to clarify an easy hypersaturation argument used repeatedly in the paper.      
...
\item[1.] Added the names of the fibrant objects to the introduction when naming the model structures in question, as well as 
%included 
a version of the table produced by the referee. % (and references to the categorical narrative).
...
\item[76.] Turned the claim that $ho(X)$ is a genuine equivariant operad into the new Proposition \ref{HMTPYGEN PROP}, and gave a full proof.
      ...
      Added an alternate formula for $\upsilon_{\**}$ after \eqref{DSETG_EQ} and Remark \ref{UPSPUSHMON REM} noting that $\upsilon_{\**}$ preserves certain pushouts. %
...
\item[81.] Added an explanation to the proof of Proposition \ref{SESP PROP} for why $X^h(J) = X(J)$. %     
      ...
      We have reorganized this section and revised proofs to increase readability. The major changes follow below.
...
\item Expanded the discussion of cube fibrancy/cofibrancy before and during the proof of Proposition \ref{JDDK PROP}, and (hopefully) made the overall argument clearer. %
...
\item Added Remark \ref{DKCOM REM} to \S 5.1, stating that DK equivalences inducing equivalences on suitable fibers (this originally took place midway through a proof in \S 5.2).
      
      Added Corollary \ref{DKCOM COR} to \S 5.1, which gives some key consequences of Remark \ref{DKCOM REM}.     
...
\item Significant rewrite of proof of Proposition \ref{COMPLE PROP} to increase readability and slightly strengthen the claims in the proof.
...
\item Added Corollary \ref{FTRIVALL COR} and moved Remark \ref{COFNOTRES REM} from later in the appendix to after Proposition \ref{FGTRL_PROP}.
      ...
      Rephrased the proofs of the key Lemmas \ref{REEDYTRCOF LEM}, \ref{REEDYTRFIB LEM} to clarify the argument.
      % and their proofs to highlight the duality and breaking of duality.
      
\item Added Notation \ref{RPLUSR NOT} to discuss the categories $\mathbb R^+(r)$ in more depth.
      
\item[58.] A brief discussion of cofibrant generation for Reedy model structures was added to the end of the appendix in Proposition \ref{REEDYCOFGEN PROP}. The proof of Proposition \ref{SDSETJRCOF PROP}, was reworded accordingly. %
...
\item[45.] Changed ``Theorem 6.1'' to ``Thm. 6.1'' when citing MW09 in Remark \ref{DESTIAL REM}
\item[46.] changed ``genuine'' and ``na\"ive'' to ``fine'' and ``coarse''.
\item[47.] Added missing dot to ``Thm. 2.1'' in the citation from \cite{Per18} (at the beggining of \S 4)
...
\item[87.] Fixed the indexes in 
%the ``extension of partial factorizations''
Lemma \ref{BLAFACT LEM}
\end{itemize}
