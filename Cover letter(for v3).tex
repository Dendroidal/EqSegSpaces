...
We have implemented many of the changes recommended by the referee.
The updates are organized into several classes:
\begin{itemize}
\item in ``Requested Revisions'', we address the two hard-stop concerns of the referee.
\item in ``Minor changes'', we list typos and other small corrections which did not affect understanding.
\item in ``Changes'', we address suggestions which required more significant edits and/or additional material to the article, or small changes which inhibited understanding.
...
\item[25.] added a reference to Goodwillie for the use of the adjective ``convex'' before the proof of Lemma \ref{CHAREDGE LEM}, and included the name ``downward closed''.
      As our argument is similar in spirit to Goodwillie's, we kept to using his nomenclature. %
...
\item[37.] I do not think the proof of Proposition \ref{HYPER PROP} should require anything analogous to the mentioned condition (b) in the proof of Lemma \ref{CHAREDGE LEM}.
      The point is that for $U \in \Omega$ and $V \hookrightarrow$ a planar face, it is $|V|<|U|$ unless $V=U$. We've added an extra sentence to Remark \ref{FACCES REM} to clarify this point. % that should make this point clearer. %
...
\item[40.] For Remark \ref{ORBHYPER_REM}: this is a matter of taste, but I find changing $T$ to $S$ in $\Lambda^E_o[T] \to \Omega[T]$ to contrast with $Sc[T] \to \Omega[T]$ to be a bit confusing. To emphasize that both of the $T$ variables are free, rather than chosen, we've instead added ``$T\in \Omega_G$'' to both expressions. %
...
\item[53.] The adjunctions from this notation are only used in a single instance, in the proof of part (i) of Proposition \ref{SSSETJREE PROP}; moreover this use is also simpler since only the single $\partial \Delta[m]$ isn't a representable.
      It feels somewhat unnecessary to fully expand on the adjunctions at this point in the discussion,
      rather than wait until Remark \ref{TWOVARADJ REM}. %
...
\item[2.] 
      Added Remark \ref{RECOVDEF REM} which explains why our results extend \cite[Thm. 6.6]{CM13a} and clarified the influence of \cite[Thm. 6.6]{CM13a} in the introduction (to the paper and to \S 4). %
...
\item[41.] added Notation \ref{ETA_NOT} to the end of \S 2.1 comparing $\Omega$ to $\mathsf{sSet}$, and introduced the stick tree $\eta$. %
...
and added Remark \ref{SEGALOP_REM} concerning the fibrant objects being the Reedy fibrant Segal operads. %
%as fibrant objects
