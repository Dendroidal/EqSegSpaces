...
\begin{itemize}
	...
	is contained in $X$;
	\item[(Ch3)] there exists a well-ordering of the $G$-cosets $I/G$ such that:
	\begin{itemize}
		\item[(i)] for all $[j] \not \geq [i]$ 
		...
		\item[(ii)] for all $i \in I$, $g\in G$, any face $V \hookrightarrow U_i$ such that $V \not \in X$ and $(V - \Xi_V) \hookrightarrow g U_i$ is contained in either $g U_i$ or some $U_k$ such that $[k]<[i]$. 
	\end{itemize}
\end{itemize}
\end{definition}
...
\begin{remark}
	...
	(Ch3) can be deduced from a much simpler condition. Namely, it suffices to check that:
	\begin{itemize}
		\item[(Ch3')] for all $i$, and writing $\bar{U}_i=\bar{U}_i^T$ for the outer closure in $T$, one has
		$\Xi_{U_i}=\Xi_{\bar{U}_i}$.
	\end{itemize}
Indeed, given (Ch3') then $V\hookrightarrow U_i$
iff $(V - \Xi_V)\hookrightarrow U_i$,
hence (Ch3.i),(Ch3.ii) hold for any well-ordering.
\end{remark}

\begin{remark}
	(Ch3) is designed to be compatible with forgetting the $G$-action. 
	Indeed, by combining the well-ordering of $I/G$ with arbitrary well-orderings of the individual cosets $[i]$, one obtains a well-ordering of $I$.
	It is straightforward to check that, after forgetting the $G$-actions, (Ch3.i) for the well-ordering of $I$ follows from the $G$-equivariant (Ch3.i),(Ch3.ii).
\end{remark}


\begin{remark}
	(Ch3) guarantees that, for fixed $\Xi$ and $X$ the characteristic conditions are monotone in $\{U_i\}$, in the sense that if it holds for some $\{U_i\}$ it also holds for all of its subsets.
...
Note that in this case the condition (Ch3.i) is vacuous.
...
Writing $K \leq G$ for the isotropy of $W$ in $\mathsf{Face}(T)$ and regarding $W \in \Omega^{K}\subseteq \Omega_{K}$, we claim that $X_C \to X_{C'}$ is then the pushout of the $G$-inner horn
\begin{equation}\label{FIRPUSH EQ}
	G \cdot_{K} 
	\left( \Lambda^{\Xi_{W}}[W] \to \Omega[W] \right),
\end{equation}
where the inner edge set $\Xi_{W}$ is $K$-equivariant due to the $G$-equivariance of $\Xi$ in (Ch0).
...
$D \not \subseteq \Xi_{W}$;
\item[(c)] the $G$-isotropy of inner faces $W - D$ with 
$D \subseteq \Xi_{W}$ 
...
$W'$ is not a face of any $V \in C$. Suppose otherwise.
Then since $W' \hookrightarrow V$ it is $ \bar{W} = \bar{W}' \hookrightarrow \bar{V}$
...
We now show (c). Suppose $g(W-D)=W-D$ for some $g \in G$.
The $G$-equivariance of $\Xi$ in (Ch0) then yields that
$g \Xi_{W-D} = \Xi_{W-D}$, and since
$\Xi_{W-D} = \Xi_{W} \setminus D$ 
(due to $D \subseteq \Xi_{W}$), 
it follows that it is also $g(W-\Xi_{W})=W-{\Xi_{W}}$.
Abbreviate $W'=W-\Xi_{W}$.
Since $W \hookrightarrow U$ it is also 
$W' \hookrightarrow U$, and thus
$gW' \hookrightarrow gU$ or, since $gW' = W'$, 
$W' \hookrightarrow gU$.
Thus $W$ meets the conditions in (Ch3.ii) and we obtain that
$W \hookrightarrow gU$ or $g^{-1} W \hookrightarrow U$.
Since $g^{-1}W' = g^{-1} W - g^{-1}\Xi_{W}$,
all of $W$, $W'=g^{-1}W'$, $g^{-1}W$ have the same outer closure $\bar{W}$.
Hence 
$g^{-1}\Xi_{W} = \Xi_{g^{-1}W} \subseteq \Xi_{\bar{W}} = \Xi_{W}$, where the last step follows since
$W \in \mathsf{Face}_{\Xi}^{lex}(U)$, and by cardinality reasons it must in fact be $\Xi_{g^{-1}W} = \Xi_{W}$.
But then $W,g^{-1}W$ are faces of $U$ with the same outer closure and the same edges, so that in fact
$W=g^{-1}W$. Thus $g\in K$, as desired.
...
But (Ch0) and (Ch3.i) for this new characteristic condition are obvious,
the new (Ch1) condition is a particular case of the original (Ch1) condition, 
the new (Ch2) condition readily follows from the original (Ch2) and (Ch3.i) conditions,
and the new (Ch3.ii) condition likewise follows from the original (Ch3.ii) condition.
...
\begin{remark}\label{CHAREDGE22 REM}
It is clear from the final paragraph of the previous proof
that one can replace the fixed edge set $\Xi$ with varying edge sets $\Xi^{[i]}$ for $[i]\in I/G$,
and likewise replacing occurrences of $\Xi$ with $\Xi^{[i]}$ in 
(Ch0),(Ch1),(Ch2),(Ch3).
\end{remark}
...
\subsection{[Workshop] The characteristic edge argument}


\begin{definition}\label{CHAREDGEPL DEF}
Let $T \in \Omega_G$, $X \subseteq \Omega[T]$ a subdendroidal set, and $\{U_i\}_{i \in I} \subseteq \mathsf{Face}(T)$ a subset.

Given a set $\Xi^i$ of inner edges of $U_i$ and a subface $V \hookrightarrow U_i$, denote $\Xi_V^i = \Xi^i \cap E^{\mathsf{i}}(V)$.

Suppose further that the indexing set $I$ is a $G$-set
and that a well-ordering of $I/G$ has been chosen.
For each orbit $[i] \in I/G$ denote
\[
...
\begin{itemize}
	...
	\item[(Ch2)] for all $i$, any face
		...
		is contained in $X_{<[i]}$;
	\item[(Ch3.1)] for all $[j] \not \geq [i]$, 
		all faces $V \hookrightarrow U_i$ such that 
		$(V-\Xi^i_V) \hookrightarrow U_j$
		are contained in $X_{<[i]}$;
	\item[(Ch3.2)] for all $i \in I$, $g\in G$,
		all faces $V \hookrightarrow U_i$ such that 
		$(V-\Xi^i_V) \hookrightarrow g U_i$
		are contained in either $g U_i$ or $X_{<[i]}$;
	\item[(Ch4)] for all $i \in I$, $g\in G$, all faces
		$V \hookrightarrow U_i$ such that 
		$gV = V$ either satisfy $g \Xi^i_V = \Xi^i_V$ or are contained in $X_{<[i]}$.
\end{itemize}
\end{definition}



{\color{red} rework this remark}

\begin{remark}\label{XIIII REM}
	For each $V \hookrightarrow U_i$, write $G_V \leq G$ for the isotropy of $V$ in $\mathsf{Face(T)}$. We set
\[
	\Xi^{[i]}_V 
	= \bigcup_{g \in G_V} \Xi^{gi}_V
	= \bigcup_{g \in G_V} g \Xi^{i}_{V}
\]
where the second equality follows 
since $\Xi^{gi}_V = g\Xi^i_{g^{-1}V} = g \Xi_V^i$
by (Ch0) and the definition of $G_V$.

By letting (Ch1),(Ch2),(Ch3.1),(Ch3.2) range over the inclusions
$V \hookrightarrow U_{gi}$ for $g \in G_V$,
one sees that these conditions are equivalent to their analogues with $\Xi^{i}_V$ replaced with $\Xi^{[i]}_V$ throughout.
\end{remark}


\begin{lemma}\label{CHAREDGEPL LEM}
If $\{\Xi^i\}_{i \in I}$ is a characteristic inner edge collection of $\{U_i\}_{i\in I}$ with respect to $X$, then the map
\begin{equation}\label{CHARLEMPL EQ}
	X \to X \cup \bigcup_{i \in I} \Omega[U_i]
\end{equation}
...
\begin{proof}
We start with the case of $I \simeq G/G_U$ orbital so that, abbreviating $U = U_{[e]}$, $\{U_i\}$ is the set of conjugates $gU$. 
Note that $G_U$ is also the isotropy of $U$ in $\mathsf{Face}(T)$.
We likewise abbreviate $\Xi_V = \Xi_V^{[e]}$ for 
$V \hookrightarrow U$.
Moreover, in this case one has $X_{<[i]}=X$ (for $[i]\in I/G$ the single $G$-orbit)
in (Ch1),(Ch2),(Ch3.2),(Ch4) and (Ch3.1) is vacuous.

We write $\mathsf{Face}_{\Xi}^{lex}(U)$
for the $G_U$-poset of (planar) faces $V \hookrightarrow U$
such that $\Xi_V \neq \emptyset$ and $\Xi_V = \Xi_{\bar{V}}$
ordered as follows: 
$V \leq V'$ if either 
	\begin{inparaenum}
...
For any $G_U$-equivariant convex subset $C$ of $\mathsf{Face}_{\Xi}^{lex}(U)$ we write
\[
X_C = 
X \cup \bigcup_{g\in G,V \in C} \Omega[gV].
\]
...
Without loss of generality we can assume that $C'$ is obtained from $C$ by adding the $G_U$-orbit of a single $W \hookrightarrow U$. Further, we may assume $W \not \in X_C$ or else $X_C=X_{C'}$.
Recalling that $G_W \leq G$ denotes the isotropy of $W$ in $\mathsf{Face}(T)$ and regarding $W \in \Omega^{G_W}\subseteq \Omega_{G_W}$, we claim that $X_C \to X_{C'}$ is then the pushout of the $G$-inner horn
\begin{equation}\label{FIRPUSH EQ}
	G \cdot_{G_W} 
	\left( \Lambda^{\Xi_{W}}[W] \to \Omega[W] \right),
\end{equation}
where the inner edge set $\Xi_{W}$ is $G_W$-equivariant by (Ch4).
...
\item[(b)] an inner face $W - D$ of $W$ is in $X_C$ iff 
$D \not \subseteq \Xi_{W}$;
\item[(c)] for those inner faces 
$W - D$ with $D \subseteq \Xi_{W}$ 
it is $G_{W-D} \leq G_W$.
...
We now show (c). Suppose $g(W-D)=W-D$ for some $g \in G$.
Since $\Xi_{W-D} = \Xi_{W} \setminus D$ 
(due to $D \subseteq \Xi_{W}$)
and the fact that by (Ch4) it is $g\Xi^i_{W-D}= \Xi^i_{W-D}$,
one obtains that it is also 
$g(W-\Xi_W)=W-\Xi_W$, and thus one reduces to showing that
$G_{W- \Xi_W} \leq G_W$ (as a side note, (Ch4) implies the reverse inequality $G_{W- \Xi_W} \geq G_W$).
Write
\[\Xi^*_W = \bigcup_{g \in G_{W-\Xi_W}} g \Xi_W.\]
$\Xi^{*}_W$ is clearly $G_{W- \Xi_W}$-equivariant,
and we further claim that all edges of $\Xi^*_W$ are inner edges of $\bar{W}$.
Indeed, since $\Xi^{[g]}_{g W} = g\Xi^{[e]}_W = g\Xi_W$ and
$gW,W$ have the same root and leaves, it suffices to show
$gW \hookrightarrow U$ or, equivalently,
$W \hookrightarrow g^{-1}U$, and this follows from (Ch3.2) since
$W-\Xi_W = g^{-1}(W-\Xi_W)\hookrightarrow g^{-1}U$.
Therefore, there is an inner face $W^{\**} \hookrightarrow \bar{W}$
such that $W^{\**} - \Xi^{\**}_W = W - \Xi_W$.
Note that $W^{\**}$ is clearly $G_{W-\Xi_W}$-equivariant since both $W^{\**} - \Xi^{\**}_W$ and $\Xi^{\**}_W$ are.
Now note that 
$\Xi_{W} \leq \Xi_{W^{\**}} \leq \Xi_{\bar{W}} = \Xi_{W}$
(where the last equality is by definition of $\mathsf{Face}_{\Xi}^{lex}(U)$) and thus $\Xi_{W^{\**}} = \Xi_{W}$,
so that in fact $W^{\**} = W$, 
establishing $G_{W-\Xi_W} \leq G_W$ as desired
(and a posteriori $\Xi_W = \Xi^{\**}_W$).
...
We now turn to the case of a general $\{U_i\}_{i \in I}$.
For each convex subset $J$ of $I/G$, set
\[
	X_J = 
	X \cup \bigcup_{g\in G,[j] \in J} \Omega[g U_j].
\]
...
By the $\{U_i\}$ orbital case, 
it now suffices to check that
$\{\Xi^{gj}\}_{gj \in [j]}$ is also a characteristic inner edge collection of $\{U_{g j}\}_{g j \in [j]}$
with respect to 
$X_J$.
(Ch0) is then clear, and since by convexity it it $X_{<[j]} \subseteq X_J$,
all of the new (Ch1),(Ch2),(Ch3.1),(Ch3.2),(Ch4)
conditions follow from the original conditions.
\end{proof}

{\color{red} HERE}

\begin{example} Consider the $\mathbb{Z}_{/2}$-tree below, where we write $\mathbb{Z}_{/2} = \{\pm 1\}$.
\[%
	\begin{tikzpicture}[auto,grow=up, every node/.style = {font=\footnotesize}]%
	\begin{scope}[level distance = 2.1em]
		...
		\node at (0,0) {$T$}%
			child{node [dummy] {}%
				child{node [dummy] {}%
					child{node [dummy] {}
						child{
						edge from parent [swap] node{$-a$}}
					edge from parent node [near end,swap] {$-c$}}%
					child{node [dummy] {}
						child{
						edge from parent node{$-b$}}
					edge from parent node [near end]{$-d$}}%
				edge from parent node [swap] {$-e$}}%
				child[sibling distance=3.5em, level distance = 2.7em]{node [dummy] {}%
					child{
					edge from parent [swap] node{$-f$}}
				edge from parent node [swap,near end] {$-g$}}%
				child[sibling distance=3.5em, level distance = 2.7em]{node [dummy] {}%
					child{
					edge from parent node {$\phantom{-}f$}}%
				edge from parent node [near end] {$\phantom{-}g$}}%
				child{node [dummy] {}%
					child{node [dummy] {}
						child{
						edge from parent node[swap]{$b\phantom{-}$}}
					edge from parent node [near end,swap] {$d\phantom{-}$}}%
					child{node [dummy] {}
						child{
						edge from parent node{$\phantom{-}a$}}
					edge from parent node [near end]{$\phantom{-}c$}}%
				edge from parent node {$\phantom{-}e$}}%
			edge from parent node [swap] {$r$}};%
	\end{scope}
	\end{tikzpicture}%
\]%
Then for the subtree $U$ depicted below we write
$E^{\mathsf{i}}(U) \supset \Xi^{+} = \{c,-d\}$ and
$E^{\mathsf{i}}(-U) \supset \Xi^{-} = \{-c,d\}$.
\[%
	\begin{tikzpicture}[auto,grow=up, every node/.style = {font=\footnotesize}]%
	\begin{scope}[level distance = 2.1em]
	\tikzstyle{level 2}=[sibling distance=5em]%
		...
		\node at (0,0) {$U$}%
			child{node [dummy] {}%
				child{node [dummy] {}%
					...
					edge from parent node [near end]{$-d$}}%
				edge from parent node [swap] {$-e$}}%
				child[sibling distance=3.5em, level distance = 2.7em]{
				edge from parent node [swap,near end] {$-g$}}%
				child[sibling distance=3.5em, level distance = 2.7em]{
				edge from parent node [near end] {$\phantom{-}f$}}%
				child{node [dummy] {}%
					child{node [dummy] {}
						child{
						edge from parent node[swap]{$b\phantom{-}$}}
		...
		\node at (6,0) {$W$}%
			child{node [dummy] {}%
				...
				child{node [dummy] {}
					...
					edge from parent node{$\phantom{-}a$}}
				edge from parent node [near end]{$\phantom{-}c$}}%
			edge from parent node [swap] {$\phantom{-}e$}};%
	\end{scope}
	\end{tikzpicture}%
\]%
We now let $X \subseteq \Omega[T]$ be the smallest subcomplex
that contains all outer faces of
$\pm U$ that \textit{contain neither of} $\pm W$.

We claim that $\{\Xi^{\pm}\}$ is a characteristic collection of
$\{\pm U\}$ with respect to $X$. (Ch0) is clear, as is (Ch1). 
Since all common faces of $U$ and $-U$ factor through with $r$ or $\pm W$, all of which are outer in both of $U$, $-U$,
(Ch3.2) follows. Moreover, the intersection $X \cap U$ is a hence a cover of $U$ and thus (Ch2) follows as well. (Ch4) is obvious since the root $r$ is the only face with non-trivial isotropy.
...
\subsection{[Workshop 2] The characteristic edge argument}
